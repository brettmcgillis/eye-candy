import{m as V}from"./splineDefaults-DEKToYgs.js";import{aB as ve,j as S,t as H,_ as z,m as $e,r as g,o as D,B as Y,af as Ce,aS as we,aE as xe,n as Me}from"./index-C4dhyc_M.js";import{s as Ae}from"./shaderMaterial-geXT8wbX.js";import{u as Ee,b as Ne,Z as Be,j as v,Y as I,v as b,f as F,X as Te,q as Re,P as Z,x as Xe,s as f,i as R,F as K,S as Ve,w as J,_ as qe}from"./three.tsl-CyZb892l.js";const Ue={splines:[V({name:"Top Wick Fire",type:"Fire",fireType:"Classic",tension:.5,closed:!1,fireWidth:.8,fireHeight:2,fireDepth:.725,fireSliceSpacing:.05,fireMagnitude:.5,fireLacunarity:4,fireGain:0,fireTintColor:"#ffffff",fireSaturation:1,fireBrightness:1.5,fireAnimated:!0,fireAnimSpeed:.5,pos:[.06,3.21,.06],points:[[0,0,0],[.01,.31,0],[.02,.65,0],[.02,1.01,0]]}),V({name:"Bottom Wick Fire",type:"Fire",fireType:"Classic",tension:.5,closed:!1,fireWidth:.8,fireHeight:2,fireDepth:.725,fireSliceSpacing:.05,fireMagnitude:.5,fireLacunarity:4,fireGain:0,fireTintColor:"#ffffff",fireSaturation:1,fireBrightness:1.5,fireAnimated:!0,fireAnimSpeed:.5,pos:[.06,-3.21,.06],points:[[0,0,0],[.01,-.31,0],[.02,-.65,0],[.02,-1.01,0]]}),V({name:"Top Wick Smoke",type:"Smoke",smokeType:"Volumetric",tension:.3,closed:!1,volParticleCount:8e3,volColor:"#b8b8b8",volOpacity:.01,volSize:1,volBlendMode:"Normal",volSpread:.35,volSpringK:1.2,volDamping:.06,volTurbulence:2,volTurbulenceSpeed:.25,volMaxDrift:2.4,flowSpeed:.04,fadeRate:4,pos:[.18,3.34,.088],points:[[0,0,0],[0,.75,0],[0,1.5,0],[0,2.25,0],[0,3,0]]}),V({name:"Bottom Wick Smoke",type:"Smoke",smokeType:"Volumetric",tension:.3,closed:!1,volParticleCount:8e3,volColor:"#b8b8b8",volOpacity:.01,volSize:1,volBlendMode:"Normal",volSpread:.35,volSpringK:1.2,volDamping:.06,volTurbulence:2,volTurbulenceSpeed:.25,volMaxDrift:2.4,flowSpeed:.04,fadeRate:4,pos:[.18,-3.34,.088],points:[[0,0,0],[0,-.75,0],[0,-1.5,0],[0,-2.25,0],[0,-3,0]]})]},Q={baseSpeed:1.15,minSpeed:.28,slowFreq:.7,slowAmp:.55,fastFreq:2.6,fastAmp:.25,microFreq:5.7,microAmp:.08,swayX:.015,swayZ:.014,pulseFreq:3.4,pulseAmp:.04,scaleX:1,scaleY:1},P=-Math.PI/4,ee={baseScale:[.8,2,.725],vertical:{cosOffset:.25,cosAmp:.25,staticNoiseAmp:.125,flowNoiseAmp:.5,pi:3.1415926},bend:{timeScale:2,heightScale:4,power:1.2,scoopFreq:.48,scoopAmp:.05,scoopCrossFreq:.36,scoopCrossPhase:1.8,scoopCrossAmp:.026,driftXFreq:.72,driftXHeightFreq:6.2,driftXAmp:.012,driftZFreq:.58,driftZHeightFreq:5.1,driftZPhase:1.2,driftZAmp:.01,signedNoiseXAmp:.016,signedNoiseZAmp:.014},alpha:{heightStart:.02,heightPeak:.16,tipFadeStart:.93,tipFadeEnd:1.02,widthBase:.62,widthTip:.12,widthTaperStart:.02,widthTaperEnd:.98,edgeNoiseXScale:5,edgeNoiseTimeScale:.35,edgeNoiseYScale:6.5,edgeNoiseTimeSpeed:1.8,edgeSoftness:.18,edgeNoiseAmp:.08},color:{blueBaseFadeStart:0,blueBaseFadeEnd:.12,blueBaseRadialStart:.18,blueBaseRadialEnd:.95,innerCoreHeightStart:.08,innerCoreHeightPeak:.22,innerCoreFadeStart:.34,innerCoreFadeEnd:.72,innerCoreRadialStart:.28,innerCoreRadialEnd:.98,warmBodyStart:.04,warmBodyEnd:.34,warmBodyFadeStart:.74,warmBodyFadeEnd:1,emberTipStart:.78,emberTipEnd:1,emberCenterStart:.08,emberCenterEnd:.65,outerLow:[1,.36,.05],outerHigh:[1,.78,.22],outerMixStart:.08,outerMixEnd:.58,blue:[.08,.18,1],blueScale:.95,core:[1,.98,.93],warm:[1,.54,.1],warmScale:.18,ember:[.92,.28,.04],emberMix:.35},shimmer:{xScale:7,timeScale:.9,yScale:5.5,timeSpeed:.6,base:.92,amp:.16},opacity:{base:.92,innerCoreBoost:.08}};function te(o){const t=new o.SphereGeometry(.5,32,32);return t.translate(0,.5,0),t}const Ze=o=>Number.isInteger(o)?`${o}.0`:`${o}`,j=o=>typeof o=="number"?Ze(o):Array.isArray(o)?o.map(t=>j(t)):o&&typeof o=="object"?Object.fromEntries(Object.entries(o).map(([t,c])=>[t,j(c)])):o,$=o=>`vec3(${o.join(", ")})`,{alpha:h,baseScale:Pe,bend:n,color:s,opacity:O,shimmer:w,vertical:T}=j(ee),ke=`
  ...
  void main() {
    ...
    pos.y *= 1.0 + (cos((posXZlen + ${T.cosOffset}) * ${T.pi}) * ${T.cosAmp}
           + noise(vec2(0.0, time)) * ${T.staticNoiseAmp}
           + noise(vec2(position.x + time, position.z + time)) * ${T.flowNoiseAmp}) * position.y;
...
`,je=`
  ...
  void main() {
    ...
    color *= shimmer;
    alpha *= ${O.base} + innerCore * ${O.innerCoreBoost};

    gl_FragColor = vec4(color, alpha);
  }
`,He=Ae({time:0},ke,je);$e({FlameMaterialImpl:He});const G=ve.forwardRef(function({side:t=H},c){return S.jsx("flameMaterialImpl",{ref:c,transparent:!0,blending:z,side:t,depthWrite:!1,toneMapped:!1})});function Le({position:o=[0,0,0],inverted:t=!1,motion:c,phaseOffset:r=0}){const e={...Q,...c},l=g.useRef(),m=g.useRef(),p=g.useRef(),a=g.useRef(0),u=g.useMemo(()=>te(Ce),[]);return D(({clock:C},i)=>{const d=C.getElapsedTime()+r,M=e.baseSpeed+Math.sin(d*e.slowFreq)*e.slowAmp+Math.sin(d*e.fastFreq+1.4)*e.fastAmp+Math.sin(d*e.microFreq)*e.microAmp;if(a.current+=i*Math.max(e.minSpeed,M),m.current&&(m.current.time=a.current),p.current&&(p.current.time=a.current),l.current){const A=Math.sin(d*3.2)*e.swayX,E=Math.cos(d*2.4+.8)*e.swayZ;l.current.rotation.x=(t?Math.PI:0)+A,l.current.rotation.z=E;const N=1+Math.sin(a.current*e.pulseFreq)*e.pulseAmp;l.current.scale.set(e.scaleX,N*e.scaleY,e.scaleX)}}),S.jsxs("group",{ref:l,position:o,rotation:t?[Math.PI,0,0]:[0,0,0],children:[S.jsxs("mesh",{"rotation-y":P,children:[S.jsx("primitive",{object:u,attach:"geometry"}),S.jsx(G,{ref:m,side:H})]}),S.jsxs("mesh",{"rotation-y":P,children:[S.jsx("primitive",{object:u,attach:"geometry"}),S.jsx(G,{ref:p,side:Y})]})]})}const q=K(([o])=>{const t=b(o).toVar();return J(Z(qe(t,b(12.9898,78.233))).mul(43758.5453123))}).setLayout({name:"flameRandom2",type:"float",inputs:[{name:"stInput",type:"vec2"}]}),x=K(([o])=>{const t=b(o).toVar(),c=Ve(t).toVar(),r=J(t).toVar(),e=q(c).toVar(),l=q(c.add(b(1,0))).toVar(),m=q(c.add(b(0,1))).toVar(),p=q(c.add(b(1,1))).toVar(),a=r.mul(r).mul(b(3,3).sub(r.mul(2))).toVar();return R(e,l,a.x).add(m.sub(e).mul(a.y).mul(F(1).sub(a.x))).add(p.sub(l).mul(a.x).mul(a.y))}).setLayout({name:"flameNoise2",type:"float",inputs:[{name:"stInput",type:"vec2"}]});function U(o){const{alpha:t,baseScale:c,bend:r,color:e,opacity:l,shimmer:m,vertical:p}=ee,a={time:Ee(0)},u=Be,C=Ne().toVarying("vFlameUv"),i=u.y.toVarying("vFlameHeight"),d=u.mul(v(...c)),M=u.xz.length(),A=I(M.add(p.cosOffset).mul(p.pi)).mul(p.cosAmp).add(x(b(0,a.time)).mul(p.staticNoiseAmp)).add(x(b(u.x.add(a.time),u.z.add(a.time))).mul(p.flowNoiseAmp)),E=d.y.mul(F(1).add(A.mul(u.y))),N=x(b(a.time.mul(r.timeScale),u.y.sub(a.time).mul(r.heightScale))).mul(2).sub(1),oe=x(b(u.y.sub(a.time).mul(r.heightScale),a.time.mul(r.timeScale))).mul(2).sub(1),L=Te(Re(i,0,1),r.power),ae=Z(a.time.mul(r.scoopFreq)),re=Z(a.time.mul(r.scoopCrossFreq).add(r.scoopCrossPhase)),se=Z(a.time.mul(r.driftXFreq).add(i.mul(r.driftXHeightFreq))).mul(r.driftXAmp),ie=I(a.time.mul(r.driftZFreq).add(i.mul(r.driftZHeightFreq)).add(r.driftZPhase)).mul(r.driftZAmp),ne=d.x.add(ae.mul(r.scoopAmp).add(N.mul(r.signedNoiseXAmp)).add(se).mul(L)),le=d.z.add(re.mul(r.scoopCrossAmp).add(oe.mul(r.signedNoiseZAmp)).add(ie).mul(L)),X=Xe(C.x.sub(.5)).mul(2),k=F(1).sub(X),me=f(t.heightStart,t.heightPeak,i).mul(F(1).sub(f(t.tipFadeStart,t.tipFadeEnd,i))),_=R(t.widthBase,t.widthTip,f(t.widthTaperStart,t.widthTaperEnd,i)),ce=x(b(X.mul(t.edgeNoiseXScale).add(a.time.mul(t.edgeNoiseTimeScale)),i.mul(t.edgeNoiseYScale).sub(a.time.mul(t.edgeNoiseTimeSpeed)))),de=F(1).sub(f(_,_.add(t.edgeSoftness).add(ce.mul(t.edgeNoiseAmp)),X)),pe=me.mul(de),ue=F(1).sub(f(e.blueBaseFadeStart,e.blueBaseFadeEnd,i)).mul(f(e.blueBaseRadialStart,e.blueBaseRadialEnd,k)),W=f(e.innerCoreHeightStart,e.innerCoreHeightPeak,i).mul(F(1).sub(f(e.innerCoreFadeStart,e.innerCoreFadeEnd,i))).mul(f(e.innerCoreRadialStart,e.innerCoreRadialEnd,k)),fe=f(e.warmBodyStart,e.warmBodyEnd,i).mul(F(1).sub(f(e.warmBodyFadeStart,e.warmBodyFadeEnd,i))),he=f(e.emberTipStart,e.emberTipEnd,i).mul(f(e.emberCenterStart,e.emberCenterEnd,X)),Se=R(v(...e.outerLow),v(...e.outerHigh),f(e.outerMixStart,e.outerMixEnd,i)),be=v(...e.blue).mul(ue).mul(e.blueScale),ge=v(...e.warm).mul(fe).mul(k).mul(e.warmScale),ye=x(b(C.x.mul(m.xScale).sub(a.time.mul(m.timeScale)),i.mul(m.yScale).add(a.time.mul(m.timeSpeed)))).mul(m.amp).add(m.base);let y=Se.add(be);y=R(y,v(...e.core),W),y=y.add(ge),y=R(y,v(...e.ember),he.mul(e.emberMix)),y=y.mul(ye);const Fe=pe.mul(F(l.base).add(W.mul(l.innerCoreBoost))),B=new we({transparent:!0,depthWrite:!1,toneMapped:!1,blending:z,side:o});return B.positionNode=v(ne,E,le),B.colorNode=y,B.opacityNode=Fe,B.uniforms=a,B}function _e({position:o=[0,0,0],inverted:t=!1,motion:c,phaseOffset:r=0}){const e={...Q,...c},l=g.useRef(),m=g.useRef(0),p=g.useMemo(()=>te(xe),[]),a=g.useMemo(()=>U(H),[]),u=g.useMemo(()=>U(Y),[]);return D(({clock:C},i)=>{const d=C.getElapsedTime()+r,M=e.baseSpeed+Math.sin(d*e.slowFreq)*e.slowAmp+Math.sin(d*e.fastFreq+1.4)*e.fastAmp+Math.sin(d*e.microFreq)*e.microAmp;if(m.current+=i*Math.max(e.minSpeed,M),a.uniforms.time.value=m.current,u.uniforms.time.value=m.current,l.current){const A=Math.sin(d*3.2)*e.swayX,E=Math.cos(d*2.4+.8)*e.swayZ;l.current.rotation.x=(t?Math.PI:0)+A,l.current.rotation.z=E;const N=1+Math.sin(m.current*e.pulseFreq)*e.pulseAmp;l.current.scale.set(e.scaleX,N*e.scaleY,e.scaleX)}}),S.jsxs("group",{ref:l,position:o,rotation:t?[Math.PI,0,0]:[0,0,0],children:[S.jsx("mesh",{"rotation-y":P,geometry:p,material:a}),S.jsx("mesh",{"rotation-y":P,geometry:p,material:u})]})}function ze(o){return Me(r=>r.gl)?.isWebGPURenderer===!0?S.jsx(_e,{...o}):S.jsx(Le,{...o})}export{Ue as B,ze as F};
