// Lets rebuild and combine the 2 outline post processing effects and combine them into one robust effect.
// The goal is to have a single effect that can be used to acheieve all the existing effects we support,
// As well as new effects, in a single reusable component.
// Use the example below as well as Outline & OutlineTSL.
// Currently OutlineTSL provides the best looking outline.
// Currently Outline provides the best looking effects on the outline.
// Currently example below contains the best effects to import,
// as well as the inside outside funtionality to import.
//
// OutlineTSL should serve as the starting point, since it is the highest quality effect we have right now.
//
// Desireable effects to include in our new effect:
// rings
// halftone
// outline
// glow
...
async function init() {
  ...
  // models
  let url =
    'https://raw.githubusercontent.com/gkjohnson/3d-demo-data/main/models/vilhelm-13/vilhelm_13.glb';
  ...
  if (window.location.hash.includes('rover')) {
    ...
    url =
      'https://raw.githubusercontent.com/gkjohnson/3d-demo-data/refs/heads/main/models/terrarium-robots/scene.gltf';
    yRotation = 0.0;
  }
...
function animate() {
  ...
  const maskWidth = Math.max(
    Math.floor(targets[0].width / params.thickness),
    3
  );
  const maskHeight = Math.max(
    Math.floor(targets[0].height / params.thickness),
    3
  );
  ...
  const delta =
    Math.max(
      Math.abs(sphere.center.x - offset.x) * width,
      Math.abs(sphere.center.y - offset.y) * height
    ) + params.thickness;
  ...
  // JFA ping pong
  let step = Math.min(
    Math.max(targets[0].width, targets[0].height),
    params.thickness
  );
  while (true) {
