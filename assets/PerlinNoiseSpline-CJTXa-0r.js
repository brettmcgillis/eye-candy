import{S as b,V as $,at as H,X as J,au as K,av as Q,a0 as N,a2 as Y,j as G,aa as S,Z as B,aw as oo,ax as O}from"./index-BnEdaNJi.js";import{n as eo}from"./splineDefaults-DyETptri.js";const to=`
${eo}
...
`,ro=[{position:[0,0,0],radius:.7},{position:[0,.9,0],radius:.65},{position:[.05,1.8,0],radius:.72},{position:[.1,2.7,.05],radius:.95},{position:[.15,3.5,.1],radius:1.25},{position:[.2,4.2,.15],radius:1.6}],U=Math.PI*2;function no(s){return Array.isArray(s)?new B(s[0],s[1],s[2]):new B(s.x??0,s.y??0,s.z??0)}function ao(s,z,m,l,L){const F=z.length,D=z.map(e=>e.radius??1),p=s.computeFrenetFrames(m,!1),d=[],T=[],M=[],v=[],y=l+1;function R(e){const o=e*(F-1),a=Math.floor(o),t=Math.min(a+1,F-1),r=o-a;return D[a]*(1-r)+D[t]*r}for(let e=0;e<=m;e++){const o=e/m,a=s.getPointAt(o),t=p.normals[e],r=p.binormals[e],c=R(o);for(let n=0;n<=l;n++){const P=n/l*U,u=Math.sin(P),A=-Math.cos(P),i=A*t.x+u*r.x,f=A*t.y+u*r.y,g=A*t.z+u*r.z;d.push(a.x+c*i,a.y+c*f,a.z+c*g),T.push(i,f,g),M.push(o)}}for(let e=0;e<m;e++)for(let o=0;o<l;o++){const a=e*y+o,t=(e+1)*y+o,r=(e+1)*y+(o+1),c=e*y+(o+1);v.push(a,t,c,t,r,c)}function E(e,o,a,t,r,c,n,P){let u=P;for(let i=1;i<=L;i++){const f=Math.PI/2*(i/L),g=r*Math.cos(f),I=n*r*Math.sin(f),_=d.length/3;for(let h=0;h<=l;h++){const j=h/l*U,C=Math.sin(j),w=-Math.cos(j),k=w*a.x+C*t.x,V=w*a.y+C*t.y,q=w*a.z+C*t.z,W=Math.cos(f)*k+Math.sin(f)*n*o.x,X=Math.cos(f)*V+Math.sin(f)*n*o.y,Z=Math.cos(f)*q+Math.sin(f)*n*o.z;d.push(e.x+I*o.x+g*k,e.y+I*o.y+g*V,e.z+I*o.z+g*q),T.push(W,X,Z),M.push(c)}for(let h=0;h<l;h++){const j=u+h,C=_+h,w=_+(h+1),k=u+(h+1);n>0?v.push(j,C,k,C,w,k):v.push(j,k,C,C,k,w)}u=_}const A=d.length/3;d.push(e.x+n*r*o.x,e.y+n*r*o.y,e.z+n*r*o.z),T.push(n*o.x,n*o.y,n*o.z),M.push(c);for(let i=0;i<l;i++)n>0?v.push(u+i,A,u+i+1):v.push(u+i,u+i+1,A)}E(s.getPointAt(0),s.getTangentAt(0),p.normals[0],p.binormals[0],R(0),0,-1,0),E(s.getPointAt(1),s.getTangentAt(1),p.normals[m],p.binormals[m],R(1),1,1,m*y);const x=new oo;return x.setIndex(v),x.setAttribute("position",new O(d,3)),x.setAttribute("normal",new O(T,3)),x.setAttribute("arcT",new O(M,1)),x}function co({controlPoints:s=ro,tubularSegments:z=128,radialSegments:m=64,capSegments:l=16,speed:L=1,weight:F=10,noiseFreq:D=.05,noiseAmp:p=5,animated:d=!0,texturePath:T="explosion.png",smokeLightColor:M="#4a4a58",smokeDarkColor:v="#1a1a22",greyscale:y=!1,position:R=[0,0,0]}){const E=b.useMemo(()=>Date.now(),[]),x=$(J,H(T)),e=b.useMemo(()=>(x.colorSpace=K,x),[x]),o=b.useMemo(()=>{const r=new Q(s.map(c=>no(c.position)),!1,"centripetal");return ao(r,s,z,m,l)},[s,z,m,l]),t=b.useRef({tExplosion:{value:e},time:{value:0},weight:{value:F},noiseFreq:{value:D},noiseAmp:{value:p},smokeLightColor:{value:new N(M)},smokeDarkColor:{value:new N(v)},greyscale:{value:y?1:0}}).current;return t.tExplosion.value=e,t.weight.value=F,t.noiseFreq.value=D,t.noiseAmp.value=p,t.smokeLightColor.value.set(M),t.smokeDarkColor.value.set(v),t.greyscale.value=y?1:0,Y(()=>{d&&(t.time.value=25e-5*L*(Date.now()-E))}),G.jsx("group",{position:R,children:G.jsx("mesh",{geometry:o,children:G.jsx("shaderMaterial",{vertexShader:to,fragmentShader:so,uniforms:t,side:S,toneMapped:!1})})})}export{co as P};
