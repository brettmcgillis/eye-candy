...
function Studio({ config }) {
  ...
  // line and falling to black overhead so the sand reads against a gradient
  ...
  // An absolute height, deliberately not derived from the sand. It used to be
  // a multiple of the sand's own depth, which meant every change to bed
  // thickness or relief silently moved the floor too. It still needs to clear
  // the bottom of the sand — buried mid-layer, the lower grains end up under
  // the floor entirely and can cast neither shadow nor occlusion onto it.
