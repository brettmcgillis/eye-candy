...
const FRAGMENT_PREAMBLE = [
  ...
  'uniform float uRippleSpeed;',
  'uniform float uRippleDecay;',
  'uniform float uRippleVisualStrength;',
  ...
  'varying float vWaveHeight;',
  '',
  'float sampleRippleRing(vec2 xz) {',
  '  float ripple = 0.0;',
  '',
  `  for (int i = 0; i < ${MAX_RIPPLES}; i++) {`,
  '    vec4 rippleData = uRippleData[i];',
  ...
  '          1.0 - smoothstep(radius * 0.35, radius, distanceToRipple);',
  '        float band =',
  '          sin(distanceToRipple * uRippleFrequency - age * uRippleSpeed) *',
  '            0.5 +',
  '          0.5;',
  '        float decay = exp(-age * uRippleDecay);',
  '',
  '        ripple = max(ripple, band * edge * decay * rippleData.w);',
  '      }',
  '    }',
  '  }',
  '',
...
const RIPPLE_OUTGOING_LIGHT_REPLACE = [
  'float ripple = max(sampleRippleRing(worldPosition.xz), vRippleHeight * 10.0);',
  'float fresnel = pow(',
  '  1.0 - max(dot(surfaceNormal, eyeDirection), 0.0),',
  ...
  '  (1.0 - surfaceNormal.y) + max(vWaveHeight, 0.0) * uFoamWaveInfluence',
  ');',
  'float rippleFoam = smoothstep(0.008, 0.06, vRippleHeight);',
  'vec3 reflectedView = normalize(reflect(-eyeDirection, surfaceNormal));',
  'vec3 waterTint = mix(',
  '  uDeepColor,',
  '  uShallowColor,',
  '  clamp(waveBand * 0.72 + (1.0 - surfaceNormal.y) * 0.2, 0.0, 1.0)',
  ');',
  ...
  '  stylizedWater,',
  '  uFoamColor,',
  '  clamp(ripple * uRippleVisualStrength, 0.0, 1.0)',
  ');',
...
export default function OceanSurface({ ocean, runtime, sun }) {
  ...
  const water = useMemo(() => {
    ...
    material.customProgramCacheKey = () => `row-it-alone-water-${MAX_RIPPLES}`;
