...
// The material always has a palette sampler bound whether or not a gradient is
// selected; `paletteMix` is the thing that goes to zero, never the binding.
...
// Marked sRGB so the renderer linearises on sample — the stops in
// gradients.json are authored in sRGB, and skipping that is what makes a
// palette read washed out over paper.
export function createPaletteTexture(name) {
  ...
  // Mirrored so paletteShift can slide past either end and fold back through
  // the gradient — these are not authored to loop, and plain repeat puts a hard
  // seam where the last stop meets the first.
