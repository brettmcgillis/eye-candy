/* eslint-disable */
import { GeomUtil, Point, Segment } from './geom';
...
export default class Analyzer {
  ...
  static getSegsAndConnections(
    ...
    const token = (pt) => {
      const t = `${Math.round(pt.x * 1)}|${Math.round(pt.y * 1)}`;
      originalPts[t] = pt;
      return t;
    };
    ...
    if (splitTeeIntersections) {
      ...
      const allPts = segs.reduce((arr, seg) => arr.concat(seg.a, seg.b), []);
      let j = allPts.length;
      ...
      while (i--) {
        ...
        allPts.forEach((pt) => {
          if (GeomUtil.distancePointSegment(pt, seg) < 0.1) {
            if (
              !GeomUtil.pointsEqual(pt, seg.a) &&
              !GeomUtil.pointsEqual(pt, seg.b)
            ) {
              crossPts.push(pt);
            }
          }
    ...
    if (splitCrossIntersections) {
      ...
      while (j--) {
        ...
        while (i--) {
          const segA = segs[j];
          const segB = segs[i];
          const intPt = GeomUtil.segmentSegmentIntersect(segA, segB, true);
          if (intPt) {
            found = true;
            segs.splice(
              j,
              1,
              new Segment(Point.clone(segA.a), Point.clone(intPt)),
              new Segment(Point.clone(intPt), Point.clone(segA.b))
            );
            segs.splice(
              i,
              1,
              new Segment(Point.clone(segB.a), Point.clone(intPt)),
              new Segment(Point.clone(intPt), Point.clone(segB.b))
            );
          }
        }
        if (found) {
    ...
    segs.forEach((seg) => {
      const ta = token(seg.a);
      const tb = token(seg.b);
      if (!cxs[ta]) cxs[ta] = [];
      if (!cxs[tb]) cxs[tb] = [];
  ...
  static pathOrder(
    ...
    const res = [];
    const { originalPts, pts, cxs } = Analyzer.getSegsAndConnections(
      segs,
      splitTeeIntersections,
      splitCrossIntersections
    );
    ...
    while (pts.length) {
      ...
      while (ptA) {
        if (cxs[ptA].length) {
          ...
          const ptB = cxs[ptA].shift();

          const oppIdx = cxs[ptB].indexOf(ptA);
  ...
  static getFills(segs, searchMultiplier = 5) {
    ...
    for (const token in originalPts) {
      const pt = originalPts[token];
      ptArray.push(pt);
    ...
    ptArray.forEach((ptA, idx) => {
      if (idx > 0) {
        const ptB = ptArray[idx - 1];
        const dx = Math.round(Math.abs(ptA.x - ptB.x));
        if (dx > 1) {
    ...
    ptArray.forEach((ptA, idx) => {
      if (idx > 0) {
        const ptB = ptArray[idx - 1];
        const dy = Math.round(Math.abs(ptA.y - ptB.y));
        if (dy > 1) {
    ...
    rayPts.forEach((rayPt) => {
      const nearPts = [];
      ptArray.forEach((pt) => {
        const dist = GeomUtil.distanceBetween(pt, rayPt);
        if (dist < Math.max(minDx, minDy) * searchMultiplier) {
          const ang = GeomUtil.angleBetween(pt, rayPt);
          nearPts.push({
            pt,
            dist,
            ang,
      ...
      while (i--) {
        const nPt = nearPts[i].pt;
        const seg = new Segment(rayPt, nPt);
        const hits = GeomUtil.segmentSegmentsIntersections(seg, segs, true);
        if (hits.length > 0) {
          nearPts.splice(i, 1);
        }
      }
      ...
      i = nearPts.length;
      while (i--) {
        const nPtA = nearPts[i].pt;
        const tokenA = token(nPtA);
        ...
        while (j--) {
          ...
          const nPtB = nearPts[j].pt;
          const tokenB = token(nPtB);
      ...
      nearPts.forEach((npA, idx) => {
        const npB = nearPts[(idx + 1) % nearPts.length];
        const tokenA = token(npA.pt);
        const tokenB = token(npB.pt);
        if (cxs[tokenA].indexOf(tokenB) === -1) {
          ok = false;
        }
      });
      if (ok) {
        const polyPts = nearPts.map((nPt) => nPt.pt);
        const cen = GeomUtil.averagePoints(...polyPts);
        const cenToken = token(cen);
