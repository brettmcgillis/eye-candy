...
export function filterBackfacing(edges, cameraPosition) {
  return edges.filter((edge) => {
    const edgeMidpoint = new Vector3()
      .addVectors(edge.a, edge.b)
      .multiplyScalar(0.5);
    const viewDir = new Vector3()
      .subVectors(cameraPosition, edgeMidpoint)
      .normalize();
...
export function findIntersection(e1, e2) {
  ...
  if (t1 > eps && t1 < 1 - eps && t2 > eps && t2 < 1 - eps) {
    return {
      t1,
      t2,
      point: new Vector2(x1 + t1 * (x2 - x1), y1 + t1 * (y2 - y1)),
    };
  }
  ...
  for (let i = 0; i < edges.length; i++) {
    for (let j = i + 1; j < edges.length; j++) {
      ...
      if (intersection) {
        ...
        splits
          .get(edges[i])
          ?.push({ t: intersection.t1, point: intersection.point });
        splits
          .get(edges[j])
          ?.push({ t: intersection.t2, point: intersection.point });
      } else {
  ...
  for (const edge of edges) {
    ...
    for (const split of edgeSplits) {
      ...
      result.push({
        ...
        b3d: point3d.clone(),
        midpoint3d: new Vector3()
          .addVectors(prevPoint3d, point3d)
          .multiplyScalar(0.5),
    ...
    result.push({
      ...
      b3d: edge.b3d.clone(),
      midpoint3d: new Vector3()
        .addVectors(prevPoint3d, edge.b3d)
        .multiplyScalar(0.5),
      isProfile: edge.isProfile,
      visible: edge.visible,
      faceIdx: edge.faceIdx,
 ...
 * @returns {Edge2D[]}
 */
export function testOcclusionDepthBuffer(
  edges,
  scene,
  camera,
  epsilon,
  ...
  for (const edge of edges) {
    ...
    const isVisible =
      Math.abs(sampledDepth - expectedDepth) < epsilon ||
      sampledDepth >= expectedDepth - epsilon;
 ...
 * @returns {Edge2D[]}
 */
export function testOcclusionFaceID(
  edges,
  meshes,
  camera,
  width,
 ...
 * @returns {boolean}
 */
function edgeLiesAlongFaceEdge(
  edgeA,
  edgeB,
  faceEdgeA,
  faceEdgeB,
 ...
 * @returns {Edge2D[]}
 */
export function filterSmoothSplitEdges(
  edges,
  projectedFaces,
  coplanarThreshold = 0.99,
...
export function testOcclusion(edges, scene, camera, epsilon = 0.05) {
  ...
  for (const edge of edges) {
    ...
    const toMidpoint = new Vector3().subVectors(
      edge.midpoint3d,
      camera.position
    );
...
export function cleanupOrphanedEdges(
  ...
  const vertexKey = (/** @type {Point2D} */ p) =>
    `${Math.round(p.x / tolerance)},${Math.round(p.y / tolerance)}`;
...
export function removeIsolatedEdges(edges, tolerance = 1.0) {
  const vertexKey = (/** @type {Vector2} */ p) =>
    `${Math.round(p.x / tolerance)},${Math.round(p.y / tolerance)}`;
...
export function computeHiddenLines(mesh, camera, scene, options = {}) {
  ...
  if (skipOcclusion) {
    ...
    const visibleOtherEdges = testOcclusionFaceID(
      otherEdges,
      [mesh],
      camera,
      width,
...
export function computeHiddenLinesMultiple(
    ...
    if (options.minHatchDotProduct !== undefined) {
      ...
      visibleHatch = visibleHatch.filter((edge) => {
        const edgeMidpoint = new Vector3()
          ...
          .multiplyScalar(0.5);
        const viewDir = new Vector3()
          .subVectors(camera.position, edgeMidpoint)
          .normalize();
  ...
  for (const mesh of meshes) {
    ...
    for (let f = 0; f < numFaces; f++) {
      ...
      const a2d = new Vector2(
        p0.x * halfWidth * internalScale,
        -p0.y * halfHeight * internalScale
      );
      const b2d = new Vector2(
        p1.x * halfWidth * internalScale,
        -p1.y * halfHeight * internalScale
      );
      const c2d = new Vector2(
        p2.x * halfWidth * internalScale,
        -p2.y * halfHeight * internalScale
      );
  ...
  return {
    ...
    projectedFaces, // For face visualization
  };
}
...
function rayIntersectsTriangle(ox, oy, rdx, rdy, maxDist, a, b, c) {
  if (raySegmentIntersect(ox, oy, rdx, rdy, maxDist, a.x, a.y, b.x, b.y))
    return true;
  if (raySegmentIntersect(ox, oy, rdx, rdy, maxDist, b.x, b.y, c.x, c.y))
    return true;
  if (raySegmentIntersect(ox, oy, rdx, rdy, maxDist, c.x, c.y, a.x, a.y))
    return true;
  return false;
}
