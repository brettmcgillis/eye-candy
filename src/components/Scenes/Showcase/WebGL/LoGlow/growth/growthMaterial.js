...
// Port of ~/dev/examples/260308_DifferentialGrowth/src/core/materialController.ts
// (mask-view path dropped — this scene has no mask painting). Colours the grown
// surface by the engine's per-vertex aCurvature/aDisplacement fields. Used for
// the 'curvature' and 'displacement' shading modes; 'neon' keeps LoGlow's own
// emissive material instead.
...
export function createGrowthMaterial(settings) {
  return new ShaderMaterial({
    ...
    uniforms: {
      ...
      uGradientType: {
        value: settings.gradientType === 'displacement' ? 1.0 : 0.0,
      },
...
export function updateGrowthMaterial(material, settings) {
  const u = material.uniforms;
  u.uGradientType.value = settings.gradientType === 'displacement' ? 1.0 : 0.0;
  u.uGradientStart.value.set(settings.gradientStart);
  u.uGradientEnd.value.set(settings.gradientEnd);
  u.uCurvatureContrast.value = settings.curvatureContrast;
  u.uCurvatureBias.value = settings.curvatureBias;
  u.uFresnel.value = settings.fresnel;
