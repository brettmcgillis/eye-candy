...
export class Optimize {
   ...
   * @returns {Segments}
   */
  static segmentCollections(
    segCols,
    noSplit = false,
    trimSmall = true,
    smallDist = 1,
    optimizePathOrder = false,
    splitTeeIntersections = false,
    splitCrossIntersections = false
  ) {
    const allsegs = segCols.reduce(
      (arr, sc) => arr.concat(sc.toSegments()),
      []
    );
   ...
   * @returns {Segments}
   */
  static segmentCollectionsPathOrder(
    segCols,
    splitTeeIntersections = false,
    splitCrossIntersections = false
  ) {
