...
function buildAutoSplatStartControls(starts, setStarts) {
  ...
  for (let i = 0; i < MAX_AUTO_SPLATS; i += 1) {
    ...
    out[autoSplatKey(i)] = {
      ...
      render: (get) => {
        const count = clampAutoSplatCount(
          get('Watercolor Squares.Interaction.AutoSplats.autoSplatCount')
        );
        return i < count;
      },
      onChange: (next) => {
  ...
  for (let i = 0; i < MAX_STATIONARY_SPLATS; i += 1) {
    ...
    out[stationarySplatKey(i)] = {
      ...
      render: (get) => {
        const count = clampStationarySplatCount(
          get(
            'Watercolor Squares.Interaction.StationarySplats.stationarySplatCount'
          )
        );
        return i < count;
      },
  ...
  for (let i = 0; i < MAX_STATIONARY_SPLATS; i += 1) {
    ...
    out[stationaryDebugMarkerKey(i)] = {
      ...
      render: (get) => {
        const count = clampStationaryDebugMarkerCount(
          get(
            'Watercolor Squares.Interaction.StationaryMarkers.stationaryDebugMarkerCount'
          )
        );
        return i < count;
      },
...
function getPresetControls({ presetSnapshot, currentControls }) {
  ...
  return {
    ...
    vorticity: p.vorticity ?? 54,
    velocityDissipation: p.velocityDissipation ?? 0.4,
    densityDissipation: p.densityDissipation ?? 1.6,
    inputMode: p.inputMode ?? 'pointer',
    dyeStrength: p.dyeStrength ?? 0.25,
    splatForce: p.splatForce ?? 4200,
    splatRadius: p.splatRadius ?? 0.0005,
    debugCursor: p.debugCursor ?? false,
    autoSplat: p.autoSplat ?? false,
    autoSplatStrength: p.autoSplatStrength ?? 0.03,
    autoSplatDyeStrength: p.autoSplatDyeStrength ?? 2.5,
    autoSplatForce: p.autoSplatForce ?? 950,
    autoSplatRadius: p.autoSplatRadius ?? 0.0005,
    autoSplatRate: p.autoSplatRate ?? 100,
    autoSplatRange: p.autoSplatRange ?? 1,
    autoSplatBurst: p.autoSplatBurst ?? 4,
    autoSplatCount: nextAutoStarts.length,
    stationarySplatsEnabled: p.stationarySplatsEnabled ?? true,
    stationarySplatStrength: p.stationarySplatStrength ?? 0.51,
    stationarySplatDyeStrength: p.stationarySplatDyeStrength ?? 0.51,
    stationarySplatForce: p.stationarySplatForce ?? 4200,
    stationarySplatRadius: p.stationarySplatRadius ?? 0.0005,
    stationarySplatDirectionStrength:
      p.stationarySplatDirectionStrength ?? 0.0005,
    stationarySplatDirectionAngle: p.stationarySplatDirectionAngle ?? 270,
    stationarySplatCount: nextSplats.length,
    stationaryDebugMarkersEnabled: p.stationaryDebugMarkersEnabled ?? true,
    stationaryDebugMarkerCount: nextMarkers.length,
    randomSplatStrength: p.randomSplatStrength ?? 1,
    randomSplatDyeStrength: p.randomSplatDyeStrength ?? 1.71,
    ...
    bgA: p.bgA ?? '#4b4b4b',
    bgB: p.bgB ?? '#797979',
    dithering: p.dithering ?? false,
    ditherStrength: p.ditherStrength ?? 2.85,
    ditherScale: p.ditherScale ?? 1,
    brightness: p.brightness ?? 1.76,
    contrast: p.contrast ?? 1.09,
    saturation: p.saturation ?? 1.64,
    debugStationarySplatColor: p.debugStationarySplatColor ?? '#000000',
    debugStationarySplatWidth: p.debugStationarySplatWidth ?? 0.05,
    debugStationarySplatHeight: p.debugStationarySplatHeight ?? 0.05,
...
function notifyArrayUpdate(
  ...
  const p = presetSnapshot;
  setAutoSplatStarts(getAutoSplatStartsFromPreset(p));
  setStationarySplats(getStationarySplatsFromPreset(p));
  setStationaryDebugMarkers(getStationaryDebugMarkersFromPreset(p));
}
...
export default function useSceneControls({ matRef, randomSplatQueueRef }) {
  const {
    ...
    getPresetControls,
    presets: WATERCOLOR_SQUARES_PRESETS,
  });

  const initialPresetSnapshot =
    WATERCOLOR_SQUARES_PRESETS[initialPreset] ||
    WATERCOLOR_SQUARES_PRESETS[DEFAULT_PRESET];
      ...
      Interaction: folder(
        {
          ...
          AutoSplats: folder(
            {
              autoSplat: { label: 'Enable', value: p.autoSplat ?? false },
              autoSplatStrength: {
                label: 'Strength',
              ...
              debugAutoColor: { label: 'Debug Color', value: '#000000' },
              debugAutoWidth: {
                label: 'Debug W',
                value: 0.03,
                min: 0.005,
          ...
          StationarySplats: folder(
            {
              stationarySplatsEnabled: {
                label: 'Enable',
                value: p.stationarySplatsEnabled ?? true,
              },
              stationarySplatStrength: {
                label: 'Strength',
                value: p.stationarySplatStrength ?? 0.51,
                min: 0,
                max: 1,
                step: 0.01,
              },
              stationarySplatDyeStrength: {
                label: 'Dye',
                value: p.stationarySplatDyeStrength ?? 0.51,
                min: 0,
                max: 2.5,
                step: 0.01,
              },
              ...
              stationarySplatDirectionStrength: {
                label: 'Dir Strength',
                value: p.stationarySplatDirectionStrength ?? 0.0005,
                min: 0,
                max: 1,
                step: 0.01,
              },
              stationarySplatDirectionAngle: {
                label: 'Dir Angle°',
                value: p.stationarySplatDirectionAngle ?? 270,
                min: 0,
                max: 360,
                step: 1,
              },
              stationarySplatCount: {
                label: 'Count',
                value: p.stationarySplatCount ?? 10,
                min: 0,
                max: 10,
                step: 1,
              },
              ...
              debugStationarySplatColor: {
                label: 'Debug Color',
                value: p.debugStationarySplatColor ?? '#000000',
              },
              debugStationarySplatWidth: {
                label: 'Debug W',
                value: p.debugStationarySplatWidth ?? 0.05,
                min: 0.005,
                max: 0.15,
                step: 0.002,
              },
              debugStationarySplatHeight: {
                label: 'Debug H',
                value: p.debugStationarySplatHeight ?? 0.05,
                min: 0.005,
                max: 0.15,
                step: 0.002,
              },
          ...
          StationaryMarkers: folder(
            {
              stationaryDebugMarkersEnabled: {
                label: 'Markers',
                value: p.stationaryDebugMarkersEnabled ?? true,
              },
              stationaryDebugMarkerCount: {
                label: 'Marker Count',
                value: p.stationaryDebugMarkerCount ?? 7,
                min: 0,
                max: 10,
                step: 1,
              },
  ...
  useEffect(() => {
    const presetSnapshot = WATERCOLOR_SQUARES_PRESETS[selectedPreset];
