...
- [x] Preserve the shared dev-page background beneath the Rorschach treatment.
- [x] Port the animated Classic Rorschach WebGL pattern as a page background.
- [x] Open Classic pattern settings from the Rorschach title icon.
- [x] Match the reference Speed, Scale, Details, Sharpness, Density, Symmetry,
      and High DPI controls.
- [x] Add live Ink and Paper color controls with Classic defaults and reset.
...
- [x] Add completed images to the output gallery while their batch is still rendering.
- [x] Keep item and collection deletion disabled until the render reaches a terminal state.
- [x] Toggle the output library between grouped batches and one flat item gallery.
- [x] Default still renders to PNG while allowing any non-empty PNG/SVG/WebP selection.
...
- [x] Show per-image preset and render stats beside media in the preview.
...
- [x] A running batch exposes completed images and metadata before the job finishes.
- [x] PNG-only, SVG-only, and WebP-only output contracts are CLI-validated.
- [x] Exclusive PNG, SVG, and WebP jobs are generated and inspected through the UI.
...
- [x] Preview stats are checked beside desktop media and stacked below mobile media.
- [x] Grouped and flat library views preserve item actions and fit desktop and mobile widths.
...
- [x] Classic background controls and reset are browser-validated at desktop
      and mobile sizes.
