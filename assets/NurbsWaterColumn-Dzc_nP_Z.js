...
${Dt}`),P.fragmentShader=P.fragmentShader.replace("#include <color_fragment>",wt)},z},[f,a,i,c,l,u]),k=b.useMemo(()=>{if(!d)return null;const z=t/2,I=e/2;return Ht(z,I,n/2,-n/2)},[d,t,n,e]),$=b.useMemo(()=>new xt({transparent:!0,depthTest:!0,depthWrite:!1,toneMapped:!1}),[]),Ne=b.useMemo(()=>new Qe(t,e,1,1),[e,t]),ve=b.useMemo(()=>n/2+Math.max(p*1.5+.048,.02),[n,p]),re=!!L,xe=b.useMemo(()=>re?at({depth:e,groupRef:Z,height:n,interactionRuntime:N,waveChoppiness:m,waveChoppinessRef:g,waveHeight:p,waveHeightRef:y,waveSpeed:v,waveSpeedRef:x,width:t}):null,[e,n,N,re,m,g,p,y,v,x,t]),ye=b.useCallback(()=>{N?.clearPointerTarget()},[N]),W=b.useCallback(z=>{if(!Z.current||!N)return;z.stopPropagation();const I=Z.current.worldToLocal(ce.current.copy(z.point));N.setPointerTarget(K.clamp(I.x,-t/2,t/2),K.clamp(I.z,-e/2,e/2))},[e,N,t]);return b.useEffect(()=>()=>j.dispose(),[j]),b.useEffect(()=>{const z=Z.current;if(z)return xe?(z.userData.lightningSurfaceType="water",z.userData.lightningTargetAdapter=xe):(delete z.userData.lightningSurfaceType,delete z.userData.lightningTargetAdapter),()=>{delete z.userData.lightningSurfaceType,delete z.userData.lightningTargetAdapter}},[xe]),Je((z,I)=>{q.current+=I;const P=y?.current??p,R=x?.current??v,O=g?.current??m,U=N?.interactionStateRef.current,Q=N?.configRef.current.enabled;if(f.uTime.value=q.current,f.uWaveHeight.value=P,f.uWaveChoppiness.value=O,f.uWaveSpeed.value=R,Ke(q.current),N?.advance(I),f.uInteractionBounds.value.set(t,e),f.uInteractionEnabled.value=Q?1:0,f.uInteractionHeightmap.value=U?.texture??j,f.uInteractionResolution.value=U?.size??1,d&&$&&($.color.set(M),$.opacity=S,$.linewidth=C,$.resolution.set(z.size.width,z.size.height)),k){const w=n/2,A=-n/2;k.topGeos.forEach(({geo:E,edge:T})=>{const B=[];for(let H=0;H<=Te;H+=1){const h=H/Te,F=T.x0+(T.x1-T.x0)*h,Y=T.z0+(T.z1-T.z0)*h,V=ge(F,Y,P,O,R),ee=Q?J(F,Y,t,e,U):0;B.push(F,w+V+ee,Y)}E.setPositions(B)}),k.vertGeos.forEach(({geo:E,cx:T,cz:B})=>{const H=ge(T,B,P,O,R),h=Q?J(T,B,t,e,U):0;E.setPositions([T,A,B,T,w+H+h,B])})}}),G.jsxs("group",{ref:Z,children:[le.map((z,I)=>G.jsx("mesh",{geometry:z,material:ie},I)),(N||re)&&G.jsx(rt,{geometry:Ne,hitRef:fe,interactionHitY:ve,onPointerMove:N?W:void 0,onPointerOut:N?ye:void 0,onPointerOver:N?W:void 0}),d&&k&&G.jsxs(G.Fragment,{children:[G.jsx("line2",{geometry:k.bottomGeo,material:$}),k.vertGeos.map(({geo:z},I)=>G.jsx("line2",{geometry:z,material:$},`v${I}`)),k.topGeos.map(({geo:z},I)=>G.jsx("line2",{geometry:z,material:$},`t${I}`))]})]})}const Ee=32,Xe=[[-1,-1],[1,-1],[1,1],[-1,1]],kt=[[0,1],[1,2],[2,3],[3,0]];function Gt(t,e,n,o,r){const s=new ft,a=[new D(-t,o,-e),new D(t,o,-e),new D(t,o,e),new D(-t,o,e),new D(-t,o,-e)];s.add(new We(new ze().setFromPoints(a),r));const i=Xe.map(([u,p])=>{const m=u*t,v=p*e,y=new Float32Array([m,o,v,m,n,v]),g=new ze;return g.setAttribute("position",new _e(y,3)),g.attributes.position.usage=ke,s.add(new We(g,r)),{geo:g,cx:m,cz:v}}),c=Xe.map(([u,p])=>({x:u*t,z:p*e})),l=kt.map(([u,p])=>{const m=c[u],v=c[p],y=Ee+1,g=new Float32Array(y*3);for(let M=0;M<y;M+=1){const S=M/Ee;g[M*3]=m.x+(v.x-m.x)*S,g[M*3+1]=n,g[M*3+2]=m.z+(v.z-m.z)*S}const x=new ze;return x.setAttribute("position",new _e(g,3)),x.attributes.position.usage=ke,s.add(new We(x,r)),{geo:x,a:m,b:v}});return{corners:i,edgeMat:r,group:s,topEdges:l}}function jt(){const t=new Float32Array([0,0,0,1]),e=new he(t,1,1,Ce,De);return e.colorSpace=we,e.generateMipmaps=!1,e.magFilter=me,e.minFilter=me,e.needsUpdate=!0,e.wrapS=pe,e.wrapT=pe,e}function Lt({width:t=3.6,depth:e=3.6,height:n=6,segments:o=24,topColor:r="#9edff0",bottomColor:s="#246f98",opacity:a=.34,transmission:i=.5,roughness:c=.3,ior:l=1.12,thickness:u=.35,waveHeight:p=.15,waveChoppiness:m=.5,waveSpeed:v=.6,waveHeightRef:y=null,waveChoppinessRef:g=null,waveSpeedRef:x=null,edgeColor:M="#1f4455",edgeOpacity:S=.65,showEdges:C=!0,interactionRuntime:d=null,lightningTarget:N=!1}){const L=b.useRef(),Z=b.useRef(),fe=b.useRef(new D),ce=b.useRef(0),q=b.useMemo(()=>jt(),[]),j=b.useMemo(()=>new ut(q),[q]),f=b.useMemo(()=>({botColor:X(new be(s)),colBot:X(-n/2),colTop:X(n/2),interactionBounds:X(new Ze(t,e)),interactionEnabled:X(0),interactionResolution:X(1),time:X(0),topColor:X(new be(r)),waveChop:X(m),waveHeight:X(p),waveSpeed:X(v)}),[s,e,n,r,m,p,v,t]);b.useEffect(()=>()=>q.dispose(),[q]);const le=b.useMemo(()=>{const W=te(()=>{const A=f.interactionBounds.x.max(_(1e-4)),E=f.interactionBounds.y.max(_(1e-4));return ae(ne.x.div(A).add(.5),_(.5).sub(ne.z.div(E)))}),z=te(()=>de(j,W()).x.mul(f.interactionEnabled)),I=te(()=>{const A=W(),E=f.interactionResolution.max(_(1)),T=_(1).div(E),B=f.interactionBounds.x.div(E).max(_(1e-4)),H=f.interactionBounds.y.div(E).max(_(1e-4)),h=de(j,A.add(ae(T.negate(),0))).x.mul(f.interactionEnabled),F=de(j,A.add(ae(T,0))).x.mul(f.interactionEnabled),Y=de(j,A.add(ae(0,T.negate()))).x.mul(f.interactionEnabled),V=de(j,A.add(ae(0,T))).x.mul(f.interactionEnabled);return Pe(Me(h.sub(F).div(B.mul(2)),1,Y.sub(V).div(H.mul(2))))}),P=te(()=>{const A=_(0).toVar();return se.forEach(({dx:E,dz:T,freq:B,amp:H})=>{const h=Ge(ae(E,T),ne.xz).mul(B).add(f.time.mul(f.waveSpeed).mul(B));A.addAssign(_(H).mul(f.waveHeight).mul(je(h)))}),A}),R=te(()=>{const A=Ie(ne.y.sub(f.colBot).div(f.colTop.sub(f.colBot)),0,1),E=Le(.5,1,A);return P().add(z()).mul(E)}),O=te(()=>{const A=_(0).toVar(),E=_(1).toVar(),T=_(0).toVar();return se.forEach(({dx:B,dz:H,freq:h,amp:F})=>{const Y=f.waveChop.div(h*F*4),V=_(h*F).mul(f.waveHeight),ee=Ge(ae(B,H),ne.xz).mul(h).add(f.time.mul(f.waveSpeed).mul(h));A.subAssign(_(B).mul(V).mul(Ye(ee))),T.subAssign(_(H).mul(V).mul(Ye(ee))),E.subAssign(Y.mul(V).mul(je(ee)))}),Pe(Me(A,E,T))}),U=te(()=>{const A=Ie(ne.y.sub(f.colBot).div(f.colTop.sub(f.colBot)),0,1),E=qe.y.greaterThan(.5).select(Le(.8,1,A),_(0)),T=O(),B=I(),H=T.y.abs().max(_(1e-4)),h=B.y.abs().max(_(1e-4)),F=T.xz.negate().div(H),Y=B.xz.negate().div(h),V=Pe(Me(F.x.add(Y.x).negate(),1,F.y.add(Y.y).negate()));return Re(qe,V,E)}),Q=te(()=>{const A=ne.y.add(R()),E=Ie(A.sub(f.colBot).div(f.colTop.sub(f.colBot)),0,1);return Re(f.botColor,f.topColor,E)}),w=new mt({transparent:!0,side:$e,depthWrite:!0});return w.color.set(r),w.attenuationColor.set(s),w.opacity=a,w.transmission=i*0,w.roughness=c,w.metalness=0,w.ior=l,w.thickness=u,w.positionNode=ne.add(Me(0,R(),0)),w.normalNode=U(),w.colorNode=Q(),w},[s,l,j,a,c,u,i,r,f]),ie=b.useMemo(()=>{const W=et({width:t,depth:e,height:n});return tt(W,o,n,Math.max(t,e))},[e,n,o,t]),k=b.useMemo(()=>{if(!C)return null;const W=new pt({color:new be(M),opacity:S,transparent:!0,depthTest:!0,depthWrite:!1,toneMapped:!1});return Gt(t/2,e/2,n/2,-n/2,W)},[e,M,S,n,C,t]),$=b.useMemo(()=>new Qe(t,e,1,1),[e,t]),Ne=b.useMemo(()=>n/2+Math.max(p*1.5+.048,.02),[n,p]),ve=!!N,re=b.useMemo(()=>ve?at({depth:e,groupRef:L,height:n,interactionRuntime:d,waveChoppiness:m,waveChoppinessRef:g,waveHeight:p,waveHeightRef:y,waveSpeed:v,waveSpeedRef:x,width:t}):null,[e,n,d,ve,m,g,p,y,v,x,t]),xe=b.useCallback(()=>{d?.clearPointerTarget()},[d]),ye=b.useCallback(W=>{if(!L.current||!d)return;W.stopPropagation();const z=L.current.worldToLocal(fe.current.copy(W.point));d.setPointerTarget(K.clamp(z.x,-t/2,t/2),K.clamp(z.z,-e/2,e/2))},[e,d,t]);return b.useEffect(()=>{const W=L.current;if(W)return re?(W.userData.lightningSurfaceType="water",W.userData.lightningTargetAdapter=re):(delete W.userData.lightningSurfaceType,delete W.userData.lightningTargetAdapter),()=>{delete W.userData.lightningSurfaceType,delete W.userData.lightningTargetAdapter}},[re]),Je((W,z)=>{ce.current+=z;const I=ce.current,P=y?.current??p,R=x?.current??v,O=g?.current??m,U=d?.interactionStateRef.current,Q=d?.configRef.current.enabled;if(Ke(I),d?.advance(z),f.time.value=I,f.waveHeight.value=P,f.waveSpeed.value=R,f.waveChop.value=O,f.colTop.value=n/2,f.colBot.value=-n/2,f.topColor.value.set(r),f.botColor.value.set(s),f.interactionBounds.value.set(t,e),f.interactionEnabled.value=Q?1:0,j.value=U?.texture??q,f.interactionResolution.value=U?.size??1,!C||!k)return;k.edgeMat.color.set(M),k.edgeMat.opacity=S;const w=n/2;k.corners.forEach(({geo:A,cx:E,cz:T})=>{const B=ge(E,T,P,O,R),H=Q?J(E,T,t,e,U):0,h=A.attributes.position,F=h.array;F[4]=w+B+H,h.needsUpdate=!0}),k.topEdges.forEach(({geo:A,a:E,b:T})=>{const B=A.attributes.position,H=B.array,h=Ee+1;for(let F=0;F<h;F+=1){const Y=F/Ee,V=E.x+(T.x-E.x)*Y,ee=E.z+(T.z-E.z)*Y,st=ge(V,ee,P,O,R),ct=Q?J(V,ee,t,e,U):0;H[F*3+1]=w+st+ct}B.needsUpdate=!0})}),G.jsxs("group",{ref:L,children:[ie.map((W,z)=>G.jsx("mesh",{geometry:W,material:le},z)),(d||ve)&&G.jsx(rt,{geometry:$,hitRef:Z,interactionHitY:Ne,onPointerMove:d?ye:void 0,onPointerOut:d?xe:void 0,onPointerOver:d?ye:void 0}),C&&k&&G.jsx("primitive",{object:k.group})]})}function Ot(t){return vt(n=>n.gl)?.isWebGPURenderer===!0?G.jsx(Lt,{...t}):G.jsx(_t,{...t})}export{He as N,ue as P,yt as a,Ot as b,ge as s,Vt as u};
