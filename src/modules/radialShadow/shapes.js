import { Fn, If, mod, select, sign, vec2 } from 'three/tsl';

// 2D SDF primitives shared by every consumer of the radial shadow map.
// Moved verbatim out of CrossTalk/utils/radialShadowTSL.js when the pipeline
// was promoted; the marker shapes are ports of Nicolas Rougier's marker SDFs
// (CrossTalk/references/markers.glsl).
...
// Segment with a round cap at each end. Not part of the occluderSDF id chain
// — adding an id there would renumber CrossTalk's baked-artwork offset and
// invalidate its presets — so consumers that want capsules call this directly.
...
// Circular arc, from IQ (https://www.shadertoy.com/view/wl23RK) via 3cKczD.
// `sc` is (sin, cos) of the half-aperture, `ra` the arc's radius, `rb` its
// thickness. Symmetric about +y, so callers rotate their point into that
// frame. One analytic primitive for a whole clean curve — the thing a chain
// of capsules can only approximate.
const sdArc = (p, sc, ra, rb) => {
  const q = vec2(p.x.abs(), p.y);
  return select(
    sc.y.mul(q.x).greaterThan(sc.x.mul(q.y)),
    q.sub(sc.mul(ra)).length(),
    q.length().sub(ra).abs()
  ).sub(rb);
};
