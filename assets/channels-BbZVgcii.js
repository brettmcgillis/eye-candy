...
`,It=xe({uTime:0,uTexture:null,uStaticAmount:0,uStaticScale:0,uStaticSpeed:0,uSnap:0,uGlitchRate:0,uScanlineStrength:0,uColorBleed:0,uCurvature:0,uVignette:0,uMaskStrength:0,uFlyback:0,uConverge:0,uBloom:0,uBreath:0,uRetrace:0,uBeamWidth:0,uChromaDrift:0,uHum:0,uThermalDrift:0,uSpotNoise:0,uMaskMode:0,uBarrelConverge:0,uPadX:0,uPadY:0},Ur,Nr);ee({CrtShowMaterial:It});function at({src:e=Ot("ren_and_stimpy.mp4"),useWebcam:t=!1,padX:r=.06,padY:n=.08,staticAmount:a=.35,staticScale:o=700,staticSpeed:u=9,snap:s=24,glitchRate:l=.18,scanlineStrength:i=.55,colorBleed:c=.14,curvature:f=.12,vignette:m=.75,maskStrength:h=.35,flybackStrength:v=.35,convergenceDrift:d=.4,bloomStrength:p=.25,breathStrength:x=.35,retraceStrength:S=.35,beamWidth:R=.5,chromaDrift:w=.3,humStrength:T=.25,barrelConvergence:k=.6,spotNoise:g=.35,thermalDrift:b=.15,maskMode:E=0,side:B=K}){const D=y.useRef();return y.useEffect(()=>{if(!D.current)return;let j=null,z=!1;const P=document.createElement("video");P.crossOrigin="anonymous",P.loop=!0,P.muted=!0,P.playsInline=!0,P.autoplay=!0;const A=()=>{if(z)return;const _=new _t(P);_.colorSpace=qe,_.minFilter=Q,_.magFilter=Q,_.generateMipmaps=!1,D.current.uTexture=_},U=async()=>{try{P.src=e,await P.play(),A()}catch(_){console.error("[CRTShowMaterial] Video failed to play:",_)}};return t?(async()=>{try{const _=navigator,q=_.mediaDevices?.getUserMedia||_.getUserMedia||_.webkitGetUserMedia||_.mozGetUserMedia;if(!q)throw new Error("getUserMedia not supported on this device");_.mediaDevices?.getUserMedia?j=await _.mediaDevices.getUserMedia({video:{facingMode:"user"},audio:!1}):j=await new Promise((Ae,Z)=>{q.call(_,{video:!0,audio:!1},Ae,Z)}),P.srcObject=j,await P.play(),A()}catch(_){console.warn("[CRTShowMaterial] Webcam failed, falling back to video:",_),U()}})():U(),()=>{z=!0,j&&j.getTracks().forEach(_=>_.stop()),D.current?.uTexture&&D.current.uTexture.dispose(),P.pause(),P.remove()}},[e,t]),G((j,z)=>{D.current&&(D.current.uTime+=z)}),C.jsx("crtShowMaterial",{ref:D,side:B,toneMapped:!1,uPadX:r,uPadY:n,uStaticAmount:a,uStaticScale:o,uStaticSpeed:u,uSnap:s,uGlitchRate:l,uScanlineStrength:i,uColorBleed:c,uCurvature:f,uVignette:m,uMaskStrength:h,uFlyback:v,uConverge:d,uBloom:p,uBreath:x,uRetrace:S,uBeamWidth:R,uChromaDrift:w,uHum:T,uThermalDrift:b,uSpotNoise:g,uMaskMode:E,uBarrelConverge:k},It.key)}const Ir=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
