...
export class Shape extends SegmentCollection {
  ...
  toSegments(local = false) {
    ...
    let start = 0;
    let len = pts.length;
    const isClockwise = GeomUtil.polygonIsClockwise(pts);
    const doReverse = isClockwise == !this.isInverted;
    if (doReverse) {
      pts = pts.reverse();
    }
    if (this.isOpen) {
    ...
    for (let i = start; i < len; i++) {
      const a = pts[i];
      const b = pts[i < pts.length - 1 ? i + 1 : 0];
...
export class PolygonShape extends Shape {
   ...
   */
  toPoints(local = false) {
    const pts = this.points ? this.points.concat() : [];
    if (!local) {
    ...
    this.points.forEach((ptA, idx) => {
      const ptB = this.points[(idx + 1) % pts.length];
      dpts = dpts.concat(
        GeomUtil.subdivideByDistance(ptA, ptB, this.divisionDistance)
      );
    });
    return dpts;
  }
...
export class CompoundShape extends Shape {
   ...
   */
  toPoints(local = false) {
    const pts = this.shapes
      .map((shape) => shape.toPoints())
      .reduce((acc, pts) => acc.concat(pts), []);
    if (!local) {
      this._makeAbsolute(pts);
    }
    return pts;
   ...
   */
  toSegments(local = false) {
    const segs = this.shapes
      .map((shape) => shape.toSegments())
      .reduce((acc, segs) => acc.concat(segs), []);
    if (!local) {
