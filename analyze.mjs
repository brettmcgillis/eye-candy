...
const walk = (node, label) => {
  ...
  for (const [k, v] of Object.entries(schema ?? {})) {
    const isFolder = v && typeof v === 'object' && (v.schema || v.type === 'FOLDER');
...
for (const [label, ctrls] of Object.entries(folders)) {
  ...
  if (!old) { console.log(`\n${label}: no pre-session preset (new technique) — keeping current`); continue; }
  const diffs = [];
  for (const [k, c] of Object.entries(ctrls)) {
    if (/Enabled$/.test(k)) continue;
    if (!(k in old)) { diffs.push(`  ${k}: (absent then) now ${JSON.stringify(now[k])}`); continue; }
    if (JSON.stringify(old[k]) === JSON.stringify(now[k])) continue;
    const bad = typeof c?.min === 'number' && (old[k] < c.min || old[k] > c.max);
    diffs.push(`  ${k}: was ${JSON.stringify(old[k])} -> now ${JSON.stringify(now[k])}${bad ? '   [OUT OF CURRENT RANGE]' : ''}`);
  }
  console.log(`\n${label}  (from "${oldName}")`);
