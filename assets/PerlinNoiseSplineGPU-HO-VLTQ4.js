import{r as C,l as Q,bq as B,T as X,aR as H,p as V,o as W,j as _,t as Z,aS as ne,V as se,aN as ye,b1 as K,b6 as re}from"./index-C4dhyc_M.js";import{F as Y,j as N,f as i,L as he,k as ze,l as Pe,i as I,u as A,ac as $,Z as S,r as ie,v as le,_ as ce,W as we}from"./three.tsl-CyZb892l.js";import{u as J}from"./Texture-BNmErylF.js";const me=`
...
`;function _e({position:o=[0,0,0],radius:z=20,detail:l=6,speed:r=1,weight:g=10,noiseFreq:c=.05,noiseAmp:x=5,texturePath:p="explosion.png",animated:T=!0,greyscale:k=!1,smokeLightColor:d="#4a4a58",smokeDarkColor:f="#1a1a22"}){const v=C.useMemo(()=>Date.now(),[]),s=Q(X,B(p)),M=C.useMemo(()=>(s.colorSpace=H,s),[s]),e=C.useRef({tExplosion:{value:M},time:{value:0},weight:{value:g},noiseFreq:{value:c},noiseAmp:{value:x},greyscale:{value:k?1:0},smokeLightColor:{value:new V(d)},smokeDarkColor:{value:new V(f)}}).current;return e.tExplosion.value=M,e.weight.value=g,e.noiseFreq.value=c,e.noiseAmp.value=x,e.greyscale.value=k?1:0,e.smokeLightColor.value.set(d),e.smokeDarkColor.value.set(f),W(()=>{T&&(e.time.value=25e-5*r*(Date.now()-v))}),_.jsxs("mesh",{position:o,children:[_.jsx("icosahedronGeometry",{args:[z,l]}),_.jsx("shaderMaterial",{vertexShader:Ce,fragmentShader:ke,uniforms:e,side:Z,toneMapped:!1})]})}const Me=6,ee=Y(([o])=>ze(N(o),Pe(1),i(2),i(.5))).setLayout({name:"signedPerlinApprox",type:"float",inputs:[{name:"input",type:"vec3"}]}),ue=Y(([o])=>{const z=N(o).toVar(),l=i(-.5).toVar();return he({start:i(1),end:i(Me),type:"float",condition:"<="},({i:r})=>{const g=i(2).pow(r).toVar(),c=ee(z.mul(g)).toVar();l.addAssign(c.abs().div(g))}),l}).setLayout({name:"approximateTurbulence",type:"float",inputs:[{name:"input",type:"vec3"}]}),Te=Y(([o,z,l])=>{const r=i(o).clamp(0,1).toVar(),g=N(z).toVar(),c=N(l).toVar(),x=I(g,c,r.mul(2).clamp(0,1)),p=I(c,c.add(N(.1,.1,.1)),r.sub(.5).mul(2).clamp(0,1));return r.lessThan(.5).select(x,p)}).setLayout({name:"smokeGradient",type:"vec3",inputs:[{name:"heatInput",type:"float"},{name:"smokeDarkColorInput",type:"vec3"},{name:"smokeLightColorInput",type:"vec3"}]});J.preload(B("explosion.png"));function Ne({position:o=[0,0,0],radius:z=20,detail:l=6,speed:r=1,weight:g=10,noiseFreq:c=.05,noiseAmp:x=5,texturePath:p="explosion.png",animated:T=!0,greyscale:k=!1,smokeLightColor:d="#4a4a58",smokeDarkColor:f="#1a1a22"}){const v=J(B(p));C.useEffect(()=>{v.colorSpace=H,v.needsUpdate=!0},[v]);const s=C.useMemo(()=>({time:A(0),weight:A(g),noiseFreq:A(c),noiseAmp:A(x),greyscale:A(k?1:0),smokeLightColor:A(new V(d)),smokeDarkColor:A(new V(f))}),[]);C.useEffect(()=>{s.weight.value=g,s.noiseFreq.value=c,s.noiseAmp.value=x,s.greyscale.value=k?1:0,s.smokeLightColor.value.set(d),s.smokeDarkColor.value.set(f)},[k,x,c,f,d,s,g]);const M=C.useMemo(()=>{const y=N(s.time,s.time,s.time),e=ue($.mul(i(.5)).add(y)),t=e.toVarying("vPerlinNoiseBallAo"),n=ee(S.mul(s.noiseFreq).add(y.mul(i(2)))),a=s.weight.mul(e).add(s.noiseAmp.mul(n)),m=t.mul(i(1.1)).add(i(1)).div(i(1.1)).clamp(0,1),P=ie(v,le(.5,m)).rgb,u=ce(P,N(.2126,.7152,.0722)),j=I(s.smokeDarkColor,s.smokeLightColor,u),h=new ne({side:Z,toneMapped:!1});return h.positionNode=S.add($.mul(a)),h.colorNode=I(P,j,s.greyscale),h},[v,s]);return W(({clock:y})=>{T&&(s.time.value=y.getElapsedTime()*.25*r)}),_.jsx("mesh",{position:o,material:M,children:_.jsx("icosahedronGeometry",{args:[z,l]})})}const fe=[{position:[0,0,0],radius:.7},{position:[0,.9,0],radius:.65},{position:[.05,1.8,0],radius:.72},{position:[.1,2.7,.05],radius:.95},{position:[.15,3.5,.1],radius:1.25},{position:[.2,4.2,.15],radius:1.6}],ae=Math.PI*2;function ve(o){return Array.isArray(o)?new se(o[0],o[1],o[2]):new se(o.x??0,o.y??0,o.z??0)}function pe(o,z,l,r,g){const c=z.length,x=z.map(e=>e.radius??1),p=o.computeFrenetFrames(l,!1),T=[],k=[],d=[],f=[],v=r+1;function s(e){const t=e*(c-1),n=Math.floor(t),a=Math.min(n+1,c-1),m=t-n;return x[n]*(1-m)+x[a]*m}for(let e=0;e<=l;e+=1){const t=e/l,n=o.getPointAt(t),a=p.normals[e],m=p.binormals[e],P=s(t);for(let u=0;u<=r;u+=1){const j=u/r*ae,h=Math.sin(j),R=-Math.cos(j),w=R*a.x+h*m.x,L=R*a.y+h*m.y,F=R*a.z+h*m.z;T.push(n.x+P*w,n.y+P*L,n.z+P*F),k.push(w,L,F),d.push(t)}}for(let e=0;e<l;e+=1)for(let t=0;t<r;t+=1){const n=e*v+t,a=(e+1)*v+t,m=(e+1)*v+(t+1),P=e*v+(t+1);f.push(n,a,P,a,m,P)}function M(e,t,n,a,m,P,u,j){let h=j;for(let w=1;w<=g;w+=1){const L=Math.PI/2*(w/g),F=m*Math.cos(L),U=u*m*Math.sin(L),O=T.length/3;for(let b=0;b<=r;b+=1){const G=b/r*ae,E=Math.sin(G),D=-Math.cos(G),q=D*n.x+E*a.x,oe=D*n.y+E*a.y,te=D*n.z+E*a.z,ge=Math.cos(L)*q+Math.sin(L)*u*t.x,xe=Math.cos(L)*oe+Math.sin(L)*u*t.y,de=Math.cos(L)*te+Math.sin(L)*u*t.z;T.push(e.x+U*t.x+F*q,e.y+U*t.y+F*oe,e.z+U*t.z+F*te),k.push(ge,xe,de),d.push(P)}for(let b=0;b<r;b+=1){const G=h+b,E=O+b,D=O+(b+1),q=h+(b+1);u>0?f.push(G,E,q,E,D,q):f.push(G,q,E,E,q,D)}h=O}const R=T.length/3;T.push(e.x+u*m*t.x,e.y+u*m*t.y,e.z+u*m*t.z),k.push(u*t.x,u*t.y,u*t.z),d.push(P);for(let w=0;w<r;w+=1)u>0?f.push(h+w,R,h+w+1):f.push(h+w,h+w+1,R)}M(o.getPointAt(0),o.getTangentAt(0),p.normals[0],p.binormals[0],s(0),0,-1,0),M(o.getPointAt(1),o.getTangentAt(1),p.normals[l],p.binormals[l],s(1),1,1,l*v);const y=new ye;return y.setIndex(f),y.setAttribute("position",new K(T,3)),y.setAttribute("normal",new K(k,3)),y.setAttribute("arcT",new K(d,1)),y}const Ae=`
${me}
