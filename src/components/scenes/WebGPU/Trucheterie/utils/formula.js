...
function Formula(string) {
  ...
  const operators = [
    //ordered by operator precedence (PEMDAS)
    ['**', (a, b) => a ** b],
    ['*', (a, b) => a * b],
    ['/', (a, b) => a / b],
    ['%', (a, b) => a % b],
  ...
  class Formula {
    ...
    getParsed() {
      const clone = (v) =>
        typeof v == 'object' ? v.map((vv) => clone(vv)) : v;
    ...
    tokenize(str) {
      ...
      nextToken: for (let i = 0; i < str.length; i++) {
        ...
        if (str.substr(i, 5) == 'Math.') {
          i += 5;
          m = new RegExp(
            `^.{${i}}(?<payload>(?<const>[A-Z][A-Z0-9]*)|(?<fn>[a-z][a-z0-9]*)).*?`
          ).exec(str);
        ...
        m = new RegExp(
          '^' + '\.'.repeat(i) + '(?<payload>[a-zA-Z_$][0-9a-zA-Z_$]*).*'
        ).exec(str);
        if (m !== null) {
        ...
        if (
          (tokens.length == 0 ||
            tokens[tokens.length - 1][0] == types.Arithmetic) &&
          (str[i] == '-' || str[i] == '+' || str[i] == '~')
        ) {
        ...
        if (str[i] == '(') {
          const isFunction =
            tokens.length > 0 && tokens[tokens.length - 1][0] == types.Function;
          let cnt = 1;
          let k = i + 1;
          let j = 0;
          let fnArgs = [];
          ...
          console.error(
            `Opened bracket at character ${i} not closed: ${str.substr(i)}`
          );
          throw new Error(
            `Opened bracket at character ${i} not closed: ${str.substr(i)}`
          );
        ...
        //Parse literal
        m = new RegExp(`^.{${i}}(?<payload>\\d+(\\.\\d+)?|\\.\\d+).*?`).exec(
          str
        );
        if (m !== null) {
        ...
        //Parse operator
        m = new RegExp(
          `^.{${i}}(?<payload>\\${operators.map((o) => o[0].split('').join('\\')).join('|\\')})`
        ).exec(str);
        if (m !== null) {
        ...
        console.error(
          `Unable to parse '${str}' because a character at ${i}: ${str[i]}`
        );
        throw new Error(
          `Unable to parse '${str}' because a character at ${i}: ${str[i]}`
        );
      ...
      return this.solveInt(
        ...
        this.#variables.reduce((a, c) => {
          ...
          if (variableMap[c] === undefined) {
            console.warn(
              `Variable ${c} not set in argument to solve() of ${this.#raw}.`
            );
            throw new Error(
              `Variable ${c} not set in argument to solve() of ${this.#raw}.`
            );
    ...
    solveInt(tokenss, variableMap) {
      ...
      for (let i = 0; i < tokens.length; i++) {
        if (tokens[i][0] == types.Function) {
          const literals = tokens[i][2].map((v) =>
            this.solveInt(v, variableMap)
          );
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (tokens[i][0] == types.Variable) {
          tokens[i] = [
            types.Literal,
            typeof variableMap[tokens[i][1]] == 'function'
              ? variableMap[tokens[i][1]](variableMap)
              : variableMap[tokens[i][1]],
          ];
        }
      }
      //Resolve unary
