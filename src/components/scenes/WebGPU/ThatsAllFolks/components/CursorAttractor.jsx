...
export default function CursorAttractor({
  ...
  const fallbackPosition = useMemo(
    () => [FALLBACK_DISTANCE, FALLBACK_DISTANCE, planeZ],
    [planeZ]
  );
  ...
  useEffect(() => {
    ...
    const resetPointer = () => {
      ...
      if (nextAttractor) {
        nextAttractor.position = [...fallbackPosition];
      }
    };
  ...
  }, [attractorStore, fallbackPosition, gl]);
  ...
  useFrame(() => {
    ...
    if (!enabled) {
      attractor.position = [...fallbackPosition];
