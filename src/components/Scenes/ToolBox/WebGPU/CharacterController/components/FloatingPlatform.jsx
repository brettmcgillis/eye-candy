  ...
  useFrame(() => {
    ...
    if (floatingMovingPlateRef.current) {
      ...
      floatingMovingPlateRef.current.setLinvel(
        movingVel.set(movingDir > 0 ? 2 : -2, floatingMovingPlateRef.current.linvel().y, 0)
      );
    }
    ...
    if (rayHit?.collider.parent()) {
      ...
      floatingPlateRef.current.applyImpulse(springDirVec.set(0, force, 0), true);
    }
    if (rayHit2?.collider.parent()) {
      const force2 =
        springK * (floatingDis - rayHit2.timeOfImpact) -
        floatingPlateRef2.current.linvel().y * dampingC;
      floatingPlateRef2.current.applyImpulse(springDirVec2.set(0, force2, 0), true);
    }
  ...
  return (
    <>
      <RigidBody position={[0, 5, -10]} mass={1} colliders={false} ref={floatingPlateRef}>
        <Html center position={[0, 2.5, 0]}><span style={{ color: 'black', fontSize: '12px', whiteSpace: 'nowrap' }}>Floating Platform — push to move</span></Html>
      ...
      <RigidBody position={[7, 5, -10]} mass={1} colliders={false} ref={floatingPlateRef2}>
        <Html center position={[0, 2.5, 0]}><span style={{ color: 'black', fontSize: '12px', whiteSpace: 'nowrap' }}>Floating Platform — push to rotate</span></Html>
        <CuboidCollider args={[2.5, 0.1, 2.5]} />
        <mesh receiveShadow castShadow>
          <boxGeometry args={[5, 0.2, 5]} />
          <meshStandardMaterial color="lightsteelblue" />
      ...
      <RigidBody position={[0, 5, -17]} mass={1} colliders={false} ref={floatingMovingPlateRef}>
        <Html center position={[0, 2.5, 0]}><span style={{ color: 'black', fontSize: '12px', whiteSpace: 'nowrap' }}>Floating & Moving Platform</span></Html>
