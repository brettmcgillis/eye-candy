  ...
  outer: for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      if (labels[y * width + x] === targetLabel) {
        ...
        const hasEdge =
          x === 0 ||
          labels[y * width + x - 1] !== targetLabel ||
          y === 0 ||
          labels[(y - 1) * width + x] !== targetLabel;
        if (hasEdge) {
          startX = x;
          startY = y;
          break outer;
...
export function debugNormalRegions(renderer, scene, camera) {
  ...
  modal.style.cssText =
    'position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;z-index:10000;cursor:pointer;';
  modal.onclick = () => modal.remove();
  canvas.style.cssText =
    'max-width:90vw;max-height:90vh;border:2px solid lime;';
  const info = document.createElement('div');
  info.style.cssText =
    'position:absolute;top:20px;left:20px;color:lime;font-family:monospace;';
