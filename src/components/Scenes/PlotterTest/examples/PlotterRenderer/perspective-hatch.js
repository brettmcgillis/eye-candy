 ...
 */
export function computeHatchDirection(
  normal,
  camera,
  screenWidth,
  screenHeight
  ...
  const screenFar = new Vector2(farScreen.x * halfW, -farScreen.y * halfH);
  ...
  if (
    Math.abs(vpProjected.x) < 100 &&
    Math.abs(vpProjected.y) < 100 &&
    vpProjected.z < 1
  ) {
    vanishingPoint = new Vector2(vpProjected.x * halfW, -vpProjected.y * halfH);
  }
...
export function generatePerspectiveHatches(region, camera, options = {}) {
  ...
  const effectiveBase =
    spacingOverride !== undefined ? spacingOverride : baseSpacing;
  ...
  const usePerspective =
    vanishingPoint &&
    Math.abs(rotationDeg) < 5 &&
    vanishingPoint.distanceTo(center) < diag * 5;
  ...
  if (usePerspective) {
    ...
    for (let i = -numLines; i <= numLines; i++) {
      ...
      const lineEnd = vanishingPoint
        .clone()
        .add(dir.clone().multiplyScalar(vpDist * 10));
    ...
    for (let i = -numLines; i <= numLines; i++) {
      ...
      const lineStart = lineCenter
        .clone()
        .add(finalDirection.clone().multiplyScalar(-diag));
      const lineEnd = lineCenter
        .clone()
        .add(finalDirection.clone().multiplyScalar(diag));
