...
export class Segment {
  ...
  static isEqual(segA, segB) {
    return (
      (GeomUtil.pointsEqual(segA.a, segB.a) &&
        GeomUtil.pointsEqual(segA.b, segB.b)) ||
      (GeomUtil.pointsEqual(segA.b, segB.a) &&
        GeomUtil.pointsEqual(segA.a, segB.b))
    );
  }
  ...
  static clone(seg) {
    return new Segment(
      new Point(seg.a.x, seg.a.y),
      new Point(seg.b.x, seg.b.y)
    );
  }
}
...
export class Segments extends SegmentCollection {
   ...
   */
  toPoints(local = false) {
    return this.toSegments(local).reduce(
      (arr, seg) => (seg ? arr.concat([seg.a, seg.b]) : arr),
      []
    );
  }
   ...
   */
  toSegments(local = false) {
    const segs = this._segments.reduce(
      (arr, seg) => (seg ? arr.concat(Segment.clone(seg)) : arr),
      []
    );
    if (!local) {
      this._makeSegsAbsolute(segs);
   ...
   */
  static clone(segs) {
    const sA = segs._segments;
    const sB = [];
    let i = sA.length;
    while (i--) {
      sB.unshift(Segment.clone(sA[i]));
    }
    const s = new Segments(sB);
...
export class GeomUtil {
  ...
  static sameAngle(segA, segB) {
    const aA = GeomUtil.angleBetween(segA.a, segA.b);
    const aB = GeomUtil.angleBetween(segB.a, segB.b);
  ...
  static sameAngleRev(segA, segB) {
    const aA = GeomUtil.angleBetween(segA.a, segA.b);
    const aB = GeomUtil.angleBetween(segB.b, segB.a);
  ...
  static outerTangents(ptA, rA, ptB, rB) {
    ...
    const angle2 = Math.acos((rA - rB) / dist);
  ...
  static pointsEqual(ptA, ptB, scale = 1) {
    return (
      Math.round((ptA.x * 10000) / scale) ==
        Math.round((ptB.x * 10000) / scale) &&
      Math.round((ptA.y * 10000) / scale) == Math.round((ptB.y * 10000) / scale)
    );
  }
  ...
  static interpolatePoints(ptA, ptB, numSegs) {
    const pts = [{ x: ptA.x, y: ptA.y }];
    const perc = 1 / numSegs;
    const deltaX = (ptB.x - ptA.x) * perc;
    const deltaY = (ptB.y - ptA.y) * perc;
  ...
  static subdivideByDistance(ptA, ptB, delta) {
    ...
    const pts = [{ x: ptA.x, y: ptA.y }];
    const dist = GeomUtil.distanceBetween(ptA, ptB);
    let perc = delta / dist;
    const numFit = Math.floor(1 / perc);
    const remain = dist % delta;
    delta += remain / numFit;
    perc = delta / dist;
    let travel = perc;
    let i = 1;
  ...
  static segmentsConnected(segA, segB, scale = 1) {
    return (
      GeomUtil.pointsEqual(segA.b, segB.a, scale) ||
      GeomUtil.pointsEqual(segA.a, segB.b, scale)
    );
  }
  ...
  static segmentSegmentIntersect(segA, segB, ignoreTouching = false) {
    ...
    if (s >= 0 && s <= 1 && t >= 0 && t <= 1) {
      const atX = x1 + t * s1_x;
      const atY = y1 + t * s1_y;
      const intPt = { x: atX, y: atY };
      if (ignoreTouching) {
        if (
          GeomUtil.pointsEqual(intPt, segB.a) ||
          GeomUtil.pointsEqual(intPt, segB.b)
        ) {
          return;
        }
        if (
          GeomUtil.pointsEqual(intPt, segA.a) ||
          GeomUtil.pointsEqual(intPt, segA.b)
        ) {
          return;
        }
      }
  ...
  static segmentSegmentsIntersections(segA, segs, ignoreTouching = false) {
    ...
    segs.forEach((seg) => {
      ...
      const intPt = GeomUtil.segmentSegmentIntersect(segA, seg, ignoreTouching);
      if (intPt) {
        pts.push(intPt);
      }
    });
  ...
  static lineSide(pt, ptA, ptB) {
    return (
      Math.round(
        ((ptB.x - ptA.x) * (pt.y - ptA.y) - (ptB.y - ptA.y) * (pt.x - ptA.x)) *
          100
      ) / 100
    );
  }
  ...
  static closestPtPointSegment(pt, seg) {
    const ab = GeomUtil.sub(seg.b, seg.a);
    const ca = GeomUtil.sub(pt, seg.a);
    let t = GeomUtil.dot(ca, ab);

    if (t < 0) {
      pt = seg.a;
    } else {
      const denom = GeomUtil.dot(ab, ab);
      if (t >= denom) {
        pt = seg.b;
      } else {
        t /= denom;
  ...
  static distancePointSegment(pt, seg) {
    return GeomUtil.distanceBetween(
      pt,
      GeomUtil.closestPtPointSegment(pt, seg)
    );
  }
  ...
  static pointWithinBoundingBox(pt, boundingBox) {
    return (
      pt.x >= boundingBox.minX &&
      pt.y >= boundingBox.minY &&
      pt.x <= boundingBox.maxX &&
      pt.y <= boundingBox.maxY
    );
  }
  ...
  static pointWithinPolygon(pt, polySegs, ignoreTouching) {
    ...
    const startPt = new Point(100000, 100000);
    const seg = new Segment(startPt, pt);

    const pts = GeomUtil.segmentSegmentsIntersections(seg, polySegs);
  ...
  static segmentWithinPolygon(seg, polySegs) {
    const aTouching = this.pointWithinPolygon(seg.a, polySegs, false);
    const bTouching = this.pointWithinPolygon(seg.b, polySegs, false);
    const aWithin = this.pointWithinPolygon(seg.a, polySegs, true);
    const bWithin = this.pointWithinPolygon(seg.b, polySegs, true);
    return (
      (aWithin && bWithin) || (aWithin && bTouching) || (bWithin && aTouching)
    );
  }
  ...
  static segmentWithinTriangle(seg, v1, v2, v3) {
    const aTouching = this.pointWithinTriangle(seg.a, v1, v2, v3, false);
    const bTouching = this.pointWithinTriangle(seg.b, v1, v2, v3, false);
    const aWithin = this.pointWithinTriangle(seg.a, v1, v2, v3, true);
    const bWithin = this.pointWithinTriangle(seg.b, v1, v2, v3, true);
    const pt = GeomUtil.averagePoints(seg.a, seg.b);
    return (
      (aWithin && bWithin) ||
      (aWithin && bTouching) ||
      (bWithin && aTouching) ||
  ...
  static polygonWithinPolygon(polySegsA, polySegsB) {
    ...
    for (let i = 0; i < polySegsA.length; i++) {
      const seg = polySegsA[i];
      const pts = GeomUtil.segmentSegmentsIntersections(seg, polySegsB);
