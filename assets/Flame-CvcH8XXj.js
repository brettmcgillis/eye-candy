...
`,je=`
  ...
  void main() {
    ...
    float shimmer = ${C.base} + noise(vec2(vUv.x * ${C.xScale} - time * ${C.timeScale}, hValue * ${C.yScale} + time * ${C.timeSpeed})) * ${C.amp};
    color *= shimmer;
    alpha *= ${I.base} + innerCore * ${I.innerCoreBoost};

    gl_FragColor = vec4(color, alpha);
  }
`,He=Ae({time:0},ke,je);$e({FlameMaterialImpl:He});const O=ve.forwardRef(function({side:t=H},c){return S.jsx("flameMaterialImpl",{ref:c,transparent:!0,blending:z,side:t,depthWrite:!1,toneMapped:!1})});function Le({position:o=[0,0,0],inverted:t=!1,motion:c,phaseOffset:r=0}){const e={...Q,...c},l=g.useRef(),m=g.useRef(),p=g.useRef(),a=g.useRef(0),u=g.useMemo(()=>te(we),[]);return D(({clock:w},i)=>{const d=w.getElapsedTime()+r,M=e.baseSpeed+Math.sin(d*e.slowFreq)*e.slowAmp+Math.sin(d*e.fastFreq+1.4)*e.fastAmp+Math.sin(d*e.microFreq)*e.microAmp;if(a.current+=i*Math.max(e.minSpeed,M),m.current&&(m.current.time=a.current),p.current&&(p.current.time=a.current),l.current){const A=Math.sin(d*3.2)*e.swayX,E=Math.cos(d*2.4+.8)*e.swayZ;l.current.rotation.x=(t?Math.PI:0)+A,l.current.rotation.z=E;const N=1+Math.sin(a.current*e.pulseFreq)*e.pulseAmp;l.current.scale.set(e.scaleX,N*e.scaleY,e.scaleX)}}),S.jsxs("group",{ref:l,position:o,rotation:t?[Math.PI,0,0]:[0,0,0],children:[S.jsxs("mesh",{"rotation-y":P,children:[S.jsx("primitive",{object:u,attach:"geometry"}),S.jsx(O,{ref:m,side:H})]}),S.jsxs("mesh",{"rotation-y":P,children:[S.jsx("primitive",{object:u,attach:"geometry"}),S.jsx(O,{ref:p,side:Y})]})]})}const q=K(([o])=>{const t=b(o).toVar();return J(Z(qe(t,b(12.9898,78.233))).mul(43758.5453123))}).setLayout({name:"flameRandom2",type:"float",inputs:[{name:"stInput",type:"vec2"}]}),x=K(([o])=>{const t=b(o).toVar(),c=Ve(t).toVar(),r=J(t).toVar(),e=q(c).toVar(),l=q(c.add(b(1,0))).toVar(),m=q(c.add(b(0,1))).toVar(),p=q(c.add(b(1,1))).toVar(),a=r.mul(r).mul(b(3,3).sub(r.mul(2))).toVar();return R(e,l,a.x).add(m.sub(e).mul(a.y).mul(F(1).sub(a.x))).add(p.sub(l).mul(a.x).mul(a.y))}).setLayout({name:"flameNoise2",type:"float",inputs:[{name:"stInput",type:"vec2"}]});function U(o){const{alpha:t,baseScale:c,bend:r,color:e,opacity:l,shimmer:m,vertical:p}=ee,a={time:Ee(0)},u=Te,w=Ne().toVarying("vFlameUv"),i=u.y.toVarying("vFlameHeight"),d=u.mul(v(...c)),M=u.xz.length(),A=G(M.add(p.cosOffset).mul(p.pi)).mul(p.cosAmp).add(x(b(0,a.time)).mul(p.staticNoiseAmp)).add(x(b(u.x.add(a.time),u.z.add(a.time))).mul(p.flowNoiseAmp)),E=d.y.mul(F(1).add(A.mul(u.y))),N=x(b(a.time.mul(r.timeScale),u.y.sub(a.time).mul(r.heightScale))).mul(2).sub(1),oe=x(b(u.y.sub(a.time).mul(r.heightScale),a.time.mul(r.timeScale))).mul(2).sub(1),L=Be(Re(i,0,1),r.power),ae=Z(a.time.mul(r.scoopFreq)),re=Z(a.time.mul(r.scoopCrossFreq).add(r.scoopCrossPhase)),se=Z(a.time.mul(r.driftXFreq).add(i.mul(r.driftXHeightFreq))).mul(r.driftXAmp),ie=G(a.time.mul(r.driftZFreq).add(i.mul(r.driftZHeightFreq)).add(r.driftZPhase)).mul(r.driftZAmp),ne=d.x.add(ae.mul(r.scoopAmp).add(N.mul(r.signedNoiseXAmp)).add(se).mul(L)),le=d.z.add(re.mul(r.scoopCrossAmp).add(oe.mul(r.signedNoiseZAmp)).add(ie).mul(L)),X=Xe(w.x.sub(.5)).mul(2),k=F(1).sub(X),me=f(t.heightStart,t.heightPeak,i).mul(F(1).sub(f(t.tipFadeStart,t.tipFadeEnd,i))),W=R(t.widthBase,t.widthTip,f(t.widthTaperStart,t.widthTaperEnd,i)),ce=x(b(X.mul(t.edgeNoiseXScale).add(a.time.mul(t.edgeNoiseTimeScale)),i.mul(t.edgeNoiseYScale).sub(a.time.mul(t.edgeNoiseTimeSpeed)))),de=F(1).sub(f(W,W.add(t.edgeSoftness).add(ce.mul(t.edgeNoiseAmp)),X)),pe=me.mul(de),ue=F(1).sub(f(e.blueBaseFadeStart,e.blueBaseFadeEnd,i)).mul(f(e.blueBaseRadialStart,e.blueBaseRadialEnd,k)),_=f(e.innerCoreHeightStart,e.innerCoreHeightPeak,i).mul(F(1).sub(f(e.innerCoreFadeStart,e.innerCoreFadeEnd,i))).mul(f(e.innerCoreRadialStart,e.innerCoreRadialEnd,k)),fe=f(e.warmBodyStart,e.warmBodyEnd,i).mul(F(1).sub(f(e.warmBodyFadeStart,e.warmBodyFadeEnd,i))),he=f(e.emberTipStart,e.emberTipEnd,i).mul(f(e.emberCenterStart,e.emberCenterEnd,X)),Se=R(v(...e.outerLow),v(...e.outerHigh),f(e.outerMixStart,e.outerMixEnd,i)),be=v(...e.blue).mul(ue).mul(e.blueScale),ge=v(...e.warm).mul(fe).mul(k).mul(e.warmScale),ye=x(b(w.x.mul(m.xScale).sub(a.time.mul(m.timeScale)),i.mul(m.yScale).add(a.time.mul(m.timeSpeed)))).mul(m.amp).add(m.base);let y=Se.add(be);y=R(y,v(...e.core),_),y=y.add(ge),y=R(y,v(...e.ember),he.mul(e.emberMix)),y=y.mul(ye);const Fe=pe.mul(F(l.base).add(_.mul(l.innerCoreBoost))),T=new Ce({transparent:!0,depthWrite:!1,toneMapped:!1,blending:z,side:o});return T.positionNode=v(ne,E,le),T.colorNode=y,T.opacityNode=Fe,T.uniforms=a,T}function We({position:o=[0,0,0],inverted:t=!1,motion:c,phaseOffset:r=0}){const e={...Q,...c},l=g.useRef(),m=g.useRef(0),p=g.useMemo(()=>te(xe),[]),a=g.useMemo(()=>U(H),[]),u=g.useMemo(()=>U(Y),[]);return D(({clock:w},i)=>{const d=w.getElapsedTime()+r,M=e.baseSpeed+Math.sin(d*e.slowFreq)*e.slowAmp+Math.sin(d*e.fastFreq+1.4)*e.fastAmp+Math.sin(d*e.microFreq)*e.microAmp;if(m.current+=i*Math.max(e.minSpeed,M),a.uniforms.time.value=m.current,u.uniforms.time.value=m.current,l.current){const A=Math.sin(d*3.2)*e.swayX,E=Math.cos(d*2.4+.8)*e.swayZ;l.current.rotation.x=(t?Math.PI:0)+A,l.current.rotation.z=E;const N=1+Math.sin(m.current*e.pulseFreq)*e.pulseAmp;l.current.scale.set(e.scaleX,N*e.scaleY,e.scaleX)}}),S.jsxs("group",{ref:l,position:o,rotation:t?[Math.PI,0,0]:[0,0,0],children:[S.jsx("mesh",{"rotation-y":P,geometry:p,material:a}),S.jsx("mesh",{"rotation-y":P,geometry:p,material:u})]})}function ze(o){return Me(r=>r.gl)?.isWebGPURenderer===!0?S.jsx(We,{...o}):S.jsx(Le,{...o})}export{Ue as B,ze as F};
