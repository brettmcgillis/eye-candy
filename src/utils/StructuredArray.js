// Packed struct-of-arrays over a storage buffer, with per-member atomic flags
// toggled at kernel build time: TSL re-runs an Fn body per shader, so each
// kernel gets the flags it set. Shared by the MLS-MPM and FLIP solvers.
...
export default class StructuredArray {
  structNode = null;
  ...
  set(index, element, value) {
    ...
    if (!Array.isArray(nextValue) || nextValue.length < member.size) return;

    for (let i = 0; i < member.size; i += 1) {
      array[offset + i] = nextValue[i];
    }
  }
  ...
  parse(layout) {
    ...
    const trailingRest = offset % 4;
    if (trailingRest !== 0) {
      offset += 4 - trailingRest;
    }
