...
`,X=B({time:0},j,N);C({FlameMaterialImpl:X});const h=w.forwardRef(function({side:t=y},a){return s.jsx("flameMaterialImpl",{ref:a,transparent:!0,side:t,depthWrite:!1,toneMapped:!1})}),W={baseSpeed:1.15,minSpeed:.28,slowFreq:.7,slowAmp:.55,fastFreq:2.6,fastAmp:.25,microFreq:5.7,microAmp:.08,swayX:.015,swayZ:.014,pulseFreq:3.4,pulseAmp:.04,scaleX:1,scaleY:1};function D({position:i=[0,0,0],inverted:t=!1,motion:a,phaseOffset:g=0}){const o={...W,...a},r=n.useRef(),m=n.useRef(),p=n.useRef(),c=n.useRef(0),u=n.useMemo(()=>{const f=new R(.5,32,32);return f.translate(0,.5,0),f},[]);return A(({clock:f},x)=>{const l=f.getElapsedTime()+g,M=o.baseSpeed+Math.sin(l*o.slowFreq)*o.slowAmp+Math.sin(l*o.fastFreq+1.4)*o.fastAmp+Math.sin(l*o.microFreq)*o.microAmp;if(c.current+=x*Math.max(o.minSpeed,M),m.current&&(m.current.time=c.current),p.current&&(p.current.time=c.current),r.current){const b=Math.sin(l*3.2)*o.swayX,F=Math.cos(l*2.4+.8)*o.swayZ;r.current.rotation.x=(t?Math.PI:0)+b,r.current.rotation.z=F;const S=1+Math.sin(c.current*o.pulseFreq)*o.pulseAmp;r.current.scale.set(o.scaleX,S*o.scaleY,o.scaleX)}}),s.jsxs("group",{ref:r,position:i,rotation:t?[Math.PI,0,0]:[0,0,0],children:[s.jsxs("mesh",{"rotation-y":d.degToRad(-45),children:[s.jsx("primitive",{object:u,attach:"geometry"}),s.jsx(h,{ref:m,side:y})]}),s.jsxs("mesh",{"rotation-y":d.degToRad(-45),children:[s.jsx("primitive",{object:u,attach:"geometry"}),s.jsx(h,{ref:p,side:k})]})]})}export{q as B,D as F};
