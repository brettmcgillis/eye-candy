  ...
  setup(builder) {
    ...
    const inShadow = (worldPos) => {
      ...
      if (this._light.isPointLight) {
        ...
        const viewZ = shadowPositionAbs.x
          .max(shadowPositionAbs.y)
          .max(shadowPositionAbs.z)
          .negate();

        const depth = viewZToPerspectiveDepth(
          viewZ,
          this._shadowCameraNear,
          this._shadowCameraFar
        );

        const result = cubeTexture(
          this._light.shadow.map.depthTexture,
          lightToPos
        ).compare(depth).r;
    ...
    const godrays = Fn(() => {
      ...
      If(inBoxDist.lessThan(0), () => {
        ...
        If(minT.equal(10000), () => {
          ...
          startPosition.assign(
            this._cameraPosition.add(minT.add(0.001).mul(direction))
          );
      ...
      If(isEarlyOut.equal(false), () => {
        ...
        Loop(samples, ({ i }) => {
          ...
          illum.addAssign(
            shadowAmount
              .mul(
                distance(startPosition, worldPosition).mul(
                  this.density.div(100)
                )
              )
              .mul(
                pow(
                  shadowInfo.y.div(this._shadowCameraFar).oneMinus(),
        ...
        output.assign(
          vec4(
            vec3(clamp(exp(illum.negate()).oneMinus(), 0, this.maxDensity)),
            depth
          )
        );
      });
