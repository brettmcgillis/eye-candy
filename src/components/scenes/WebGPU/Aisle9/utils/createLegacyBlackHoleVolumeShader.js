  ...
  return Fn(
    ([hitPosition, hitRadius, hitAngle, rayDirection, diskTextureNode]) => {
      ...
      If(uniforms.diskVariant.greaterThan(float(2.5)), () => {
        ...
        // ── getColor ──────────────────────────────────────────────────────────
        const scaledPos = hitPosition.div(
          max(uniforms.ribbonBandScale, float(0.001))
        );
        ...
        const shadNorm = normalize(
          vec3(diskDirX.mul(float(1.2)), diskDirZ.mul(float(1.2)), float(1))
        );
        ...
        const tcol = clamp(
          ribbonCol.mul(ribbonCol).mul(float(2)),
          float(0),
          float(1)
        );
        ...
        const finalRibbonCol = clamp(
          combined.mul(combined).mul(float(2)),
          float(0),
          float(1)
        );

        // ── edge fades ────────────────────────────────────────────────────────
        const ribbonOuterFade = smoothstep(
          float(1.0),
          float(0.8),
          normalizedRadius
        );
        const ribbonInnerFade = smoothstep(
          float(0.0),
          float(0.08),
          normalizedRadius
        );
