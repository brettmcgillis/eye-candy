...
`,Pe=G({uScene:null,uFeedback:null,uTime:0,uDecay:.85,uZoom:1.01,uWarp:.6,uStaticAmount:.04,uScanlineStrength:.4,uCurvature:.12,uVignette:.85},De,Fe);W({CrtAccumMaterial:Pe});function Ve({resolution:e=1024,decay:t=.85,zoom:u=1.01,warp:n=.6,staticAmount:s=.04,scanlineStrength:c=.4,curvature:i=.12,vignette:o=.85,side:m=L}){const a=f.useRef(),{gl:r,scene:p,camera:h}=q(),v=f.useMemo(()=>new O(e,e),[e]),d=f.useMemo(()=>new O(e,e),[e]),g=f.useMemo(()=>new O(e,e),[e]),x=f.useRef(!1);return j((M,R)=>{if(!a.current)return;a.current.uTime+=R;const B=r.getRenderTarget();r.setRenderTarget(v),r.clear(),r.render(p,h);const C=x.current?d:g,T=x.current?g:d;a.current.uScene=v.texture,a.current.uFeedback=C.texture,r.setRenderTarget(T),r.clear(),r.render(p,h),r.setRenderTarget(B),x.current=!x.current,a.current.uFeedback=T.texture}),l.jsx("crtAccumMaterial",{ref:a,side:m,uDecay:t,uZoom:u,uWarp:n,uStaticAmount:s,uScanlineStrength:c,uCurvature:i,uVignette:o,toneMapped:!1})}const Ue=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
...
`,je=G({uMap:null,uTime:0,uStaticAmount:.1,uStaticScale:600,uStaticSpeed:6,uScanlineStrength:.4,uCurvature:.12,uVignette:.85,uChromaDrift:.25,uBloom:.25},Ue,Ee);W({CrtSceneShaderMaterial:je});function Ne({scene:e,resolution:t=1024,staticAmount:u=.12,staticScale:n=600,staticSpeed:s=6,scanlineStrength:c=.4,curvature:i=.12,vignette:o=.85,chromaDrift:m=.25,bloom:a=.25,side:r=L}){const p=f.useRef();return j((h,v)=>{p.current&&(p.current.uTime+=v)}),l.jsx("crtSceneShaderMaterial",{ref:p,side:r,uStaticAmount:u,uStaticScale:n,uStaticSpeed:s,uScanlineStrength:c,uCurvature:i,uVignette:o,uChromaDrift:m,uBloom:a,toneMapped:!1,children:l.jsx(be,{attach:"uMap",frames:1/0,width:t,height:t,anisotropy:8,children:e})})}const Le=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
...
`,re=G({uTime:0,uTexture:null,uStaticAmount:0,uStaticScale:0,uStaticSpeed:0,uSnap:0,uGlitchRate:0,uScanlineStrength:0,uColorBleed:0,uCurvature:0,uVignette:0,uMaskStrength:0,uFlyback:0,uConverge:0,uBloom:0,uBreath:0,uRetrace:0,uBeamWidth:0,uChromaDrift:0,uHum:0,uThermalDrift:0,uSpotNoise:0,uMaskMode:0,uBarrelConverge:0,uPadX:0,uPadY:0},Le,We);W({CrtShowMaterial:re});function J({src:e=me("ren_and_stimpy.mp4"),useWebcam:t=!1,padX:u=.06,padY:n=.08,staticAmount:s=.35,staticScale:c=700,staticSpeed:i=9,snap:o=24,glitchRate:m=.18,scanlineStrength:a=.55,colorBleed:r=.14,curvature:p=.12,vignette:h=.75,maskStrength:v=.35,flybackStrength:d=.35,convergenceDrift:g=.4,bloomStrength:x=.25,breathStrength:M=.35,retraceStrength:R=.35,beamWidth:B=.5,chromaDrift:C=.3,humStrength:T=.25,barrelConvergence:w=.6,spotNoise:y=.35,thermalDrift:U=.15,maskMode:F=0,side:E=L}){const A=f.useRef();return f.useEffect(()=>{if(!A.current)return;let P=null,I=!1;const k=document.createElement("video");k.crossOrigin="anonymous",k.loop=!0,k.muted=!0,k.playsInline=!0,k.autoplay=!0;const $=()=>{if(I)return;const b=new ve(k);b.colorSpace=te,b.minFilter=N,b.magFilter=N,b.generateMipmaps=!1,A.current.uTexture=b},z=async()=>{try{k.src=e,await k.play(),$()}catch(b){console.error("[CRTShowMaterial] Video failed to play:",b)}};return t?(async()=>{try{const b=navigator,X=b.mediaDevices?.getUserMedia||b.getUserMedia||b.webkitGetUserMedia||b.mozGetUserMedia;if(!X)throw new Error("getUserMedia not supported on this device");b.mediaDevices?.getUserMedia?P=await b.mediaDevices.getUserMedia({video:{facingMode:"user"},audio:!1}):P=await new Promise((oe,ue)=>{X.call(b,{video:!0,audio:!1},oe,ue)}),k.srcObject=P,await k.play(),$()}catch(b){console.warn("[CRTShowMaterial] Webcam failed, falling back to video:",b),z()}})():z(),()=>{I=!0,P&&P.getTracks().forEach(b=>b.stop()),A.current?.uTexture&&A.current.uTexture.dispose(),k.pause(),k.remove()}},[e,t]),j((P,I)=>{A.current&&(A.current.uTime+=I)}),l.jsx("crtShowMaterial",{ref:A,side:E,toneMapped:!1,uPadX:u,uPadY:n,uStaticAmount:s,uStaticScale:c,uStaticSpeed:i,uSnap:o,uGlitchRate:m,uScanlineStrength:a,uColorBleed:r,uCurvature:p,uVignette:h,uMaskStrength:v,uFlyback:d,uConverge:g,uBloom:x,uBreath:M,uRetrace:R,uBeamWidth:B,uChromaDrift:C,uHum:T,uThermalDrift:U,uSpotNoise:y,uMaskMode:F,uBarrelConverge:w},re.key)}const Ge=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
